import{r as e}from"./behavior-runtime-B_fkMmgZ.js";var t=e.theme;function n(e,n=0){let r=t.ornaments[e],[,,i,a]=r.viewBox.split(` `).map(Number),o=r.paths.map(e=>`<path d="${e.d}" fill="${e.fill===`none`?`none`:t.colors.carvingInk}" fill-rule="evenodd"/>`).join(``),s=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="${r.viewBox}" stroke="${t.colors.jadeCarving}" stroke-width="1.5" fill="none"><g transform="rotate(${n} ${i/2} ${a/2})">${o}</g></svg>`;return`url("data:image/svg+xml,${encodeURIComponent(s).replace(/'/g,`%27`)}")`}var r=[`--ui-motif-disc:${n(`disc`)}`,`--ui-motif-seal:${n(`seal`)}`,...[`nw`,`ne`,`se`,`sw`].map((e,t)=>`--ui-motif-corner-${e}:${n(`corner`,t*90)}`)].join(`;`)+`;`,i=e.theme,a=e=>e.replace(/[A-Z]/g,e=>`-${e.toLowerCase()}`),o=[...Object.entries(i.colors).map(([e,t])=>`--ui-${a(e)}:${t}`),...Object.entries(i.fonts).map(([e,t])=>`--ui-font-${e}:${t}`),...Object.entries(i.space).map(([e,t])=>`--ui-space-${e}:${t}px`),...Object.entries(i.shape).map(([e,t])=>`--ui-${e}:${t}px`),...Object.entries(i.motion).map(([e,t])=>`--ui-${a(e)}:${t}ms`),`--ui-rainbow:linear-gradient(120deg,${i.rainbow.join(`,`)})`,`--ui-line:color-mix(in srgb,var(--ui-bronze) 40%,transparent)`,`--ui-panel:color-mix(in srgb,var(--ui-surface) 94%,transparent)`].join(`;`)+`;`+r,s=String.raw`
