...
fn waterContact(c: vec4<f32>, shape: vec4<f32>, worldXZ_1: vec2<f32>, edgeAA: f32) -> vec3<f32> {
    ...
    let radius = (shape.x + (age * (1.45f + (shape.w * 0.65f))));
    if (dot(delta, delta) > ((radius + 0.65f) * (radius + 0.65f))) {
        return vec3(0f);
    }
    let distance_ = max(0.001f, length(delta));
    let crest = (distance_ - radius);
    ...
    let slope_1 = (((((delta / vec2(distance_)) * sin((crest * 15f))) * envelope) * fade) * 0.2f);
    let stroke = (1f - smoothstep(0.045f, (0.11f + edgeAA), abs(crest)));
    return vec3<f32>(slope_1, ((stroke * fade) * bow));
}
...
fn fs_main(input_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e220 = slope.x;
    let _e223 = slope.y;
    let normal_2 = normalize(vec3<f32>(-(_e220), 1f, -(_e223)));
    ...
    color = (_e281 * (vec3<f32>(0.56f, 0.65f, 0.7f) + (_e288 * ((0.27f * max(0f, dot(normal_2, sun))) * _e245))));
    let fresnel_1 = (0.02f + (0.98f * pow((1f - max(0f, dot(normal_2, camera))), 5f)));
    ...
    let highlight = pow(max(0f, dot(normal_2, halfVector)), 96f);
    let _e358 = color;
