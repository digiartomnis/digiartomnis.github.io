...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e50 = frame.invvp;
    let origin = (_e50 * vec4<f32>(native.center, 1f));
    let _e62 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    let _e65 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(((origin.xyz / vec3(origin.w)) - _e62.xyz));
    p.position = vec4<f32>(_e65, 1f);
    ...
    let _e104 = p.position;
    let _e108 = frame.motion;
    let _e113 = frame.settings.x;
    center = (_e104.xyz + (_e108.xyz * _e113));
    let _e118 = p.size_rotation;
    size = _e118.xy;
    let _e124 = frame.layer.x;
    let columns = u32(_e124);
    let _e129 = frame.layer.y;
    let rows = u32(_e129);
    let _e133 = p.velocity.w;
    let tile = min(u32(max(_e133, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e155 = frame.extra.x;
    let _e158 = p.id;
    let _e160 = dm_random(bitcast<u32>(_e155), _e158, 210u);
    let _e170 = frame.extra.x;
    let _e173 = p.id;
    let _e175 = dm_random(bitcast<u32>(_e170), _e173, 211u);
    let uvCorner = vec2<f32>(select(c_2.x, -(c_2.x), (_e160 < 0f)), select(c_2.y, -(c_2.y), (_e175 < 0f)));
    let _e186 = frame.layer;
    let _e189 = p;
    let _e193 = frame.extra.x;
    let _e195 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e186.xy), _e189, bitcast<u32>(_e193));
    let _e196 = size;
    let _e199 = p.size_rotation.w;
    let meshSize = vec3<f32>(_e196, _e199);
    let _e204 = p.size_rotation.z;
    let angles = (vec3<f32>(_e195.rotationXY, -(_e204)) * 57.29578f);
    let _e212 = dm_rotate((mv.position.xyz * meshSize), angles);
    let forward = vec3<f32>(0f, -1f, -0.00000005321248f);
    let _e224 = dm_rotate((mv.normal.xyz / max(abs(meshSize), vec3(0.00001f))), angles);
    let _e227 = right;
    let _e230 = up;
    let normal = normalize((((_e227 * _e224.x) + (_e230 * _e224.y)) + (forward * _e224.z)));
    let _e238 = center;
    let _e239 = right;
    let _e242 = up;
    let _e252 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e238 + ((((_e239 * _e212.x) + (_e242 * _e212.y)) + (forward * _e212.z)) * _e252));
    input.uv0_ = _e195.uv0_;
    input.custom1_ = _e195.uv1_;
    let _e263 = p.color;
    input.color = (_e263 * mv.color);
    input.normal = normal;
    let _e268 = world;
    input.worldPosition = _e268;
    let _e272 = frame.eye;
    let _e274 = world;
    input.viewDirection = (_e272.xyz - _e274);
    let _e280 = frame.settings.x;
    input.time = _e280;
    input.delta = 0.008333334f;
    let _e283 = input;
    let _e284 = source_vertex(_e283);
    let _e286 = p.age;
    let _e287 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e284, _e286);
    let _e290 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e287 + _e290.xyz);
    let _e297 = frame.vp;
    let _e298 = world;
    o.position = (_e297 * vec4<f32>(_e298, 1f));
    o.uv = _e195.uv0_;
    o.custom = _e195.uv1_;
    let _e308 = p.color;
    o.color = (_e308 * mv.color);
    let _e312 = world;
    o.world = _e312;
    let _e316 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e320 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e324 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e316.xyz, _e320.xyz, _e324.xyz) * normal));
    let _e329 = o;
    return _e329;
}
