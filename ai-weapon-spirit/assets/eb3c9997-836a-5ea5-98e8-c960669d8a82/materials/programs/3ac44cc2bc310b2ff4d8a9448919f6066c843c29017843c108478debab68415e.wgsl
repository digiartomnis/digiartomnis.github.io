            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e217 = x;
                        let _e219 = y;
                        let offsetUv = clamp((uv_2 + (vec2<f32>(f32(_e217), f32(_e219)) * texel)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e253 = x_1;
                    let _e255 = y_1;
                    let offsetUv_1 = clamp((uv_2 + (vec2<f32>(f32(_e253), f32(_e255)) * texel)), tileLo, tileHi);
...
fn enchantedSurface(base: vec3<f32>, world: vec3<f32>, normal_2: vec3<f32>, viewDirection: vec3<f32>, patternNormal_1: vec3<f32>, height01_: f32, time: f32, pixelSize_1: f32, radiance: vec3<f32>, shadow_1: f32) -> vec3<f32> {
    var surface: vec3<f32>;
    var emission: vec3<f32> = vec3(0f);

    let n = normalize(normal_2);
