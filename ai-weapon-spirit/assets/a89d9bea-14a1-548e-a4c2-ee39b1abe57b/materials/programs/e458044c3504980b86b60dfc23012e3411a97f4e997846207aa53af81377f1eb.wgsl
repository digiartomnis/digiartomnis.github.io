...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_3: u32, count_4: u32, normal_2: vec3<f32>, l: vec3<f32>, useNormalBias: bool) -> f32 {
    var local_3: bool;
    var local_4: bool;
    var local_5: bool;
    var local_6: bool;
    ...
    let bias = select(_e33, max((_e36 * (1f - dot(normal_2, l))), _e46), useNormalBias);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_3: vec3<f32>, worldPos_1: vec3<f32>, viewZ: f32) -> f32 {
    var shadow: f32;
    var local_8: bool;
    ...
    let _e32 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1, _e28, count_5, normal_3, l_1, true);
    shadow = _e32;
    ...
    if _e45 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t > 0f) {
                let _e69 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1, (_e28 + 1u), count_5, normal_3, l_1, true);
...
fn illustratedDiffuseX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU2LMNR2XG5DSMF2GKZC7NRUWO2DUNFXGOX(surface: vec3<f32>, shadowPigment: vec3<f32>, normal_4: vec3<f32>, visibility: f32, sideShade: f32, continuous: bool) -> vec3<f32> {
    var diffuse: f32;

    let _e3 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let nDotL_2 = dot(normal_4, normalize(-(_e3)));
    let up = clamp(normal_4.y, 0f, 1f);
...
fn paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(base: vec3<f32>, normal_5: vec3<f32>, light: vec3<f32>, camera: vec3<f32>, roughness_2: f32, metallic: f32, strength: f32, visibility_1: f32) -> vec3<f32> {
    let halfVector = normalize((light + camera));
    let smoothness = (1f - clamp(roughness_2, 0.04f, 1f));
    let lobe = pow(max(dot(normal_5, halfVector), 0f), mix(12f, 112f, smoothness));
    let reflection = (((((strength * smoothness) * smoothness) * lobe) * max(dot(normal_5, light), 0f)) * visibility_1);
    return (mix(vec3<f32>(0.88f, 0.86f, 0.81f), base, metallic) * reflection);
}

fn paintedRimX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(normal_6: vec3<f32>, camera_1: vec3<f32>, roughness_3: f32, strength_1: f32, visibility_2: f32) -> f32 {
    let fresnel_1 = pow((1f - clamp(dot(normal_6, camera_1), 0f, 1f)), 4f);
...
fn surfaceValueNoise(point: vec2<f32>) -> f32 {
    ...
    let local_9 = fract(point);
    let blend = (((local_9 * local_9) * local_9) * ((local_9 * ((local_9 * 6f) - vec2(15f))) + vec2(10f)));
...
fn surfaceFilteredNoise(point_1: vec2<f32>) -> f32 {
    ...
    let footprint = max(length(_e1), length(_e3));
    let resolved = (1f - smoothstep(0.18f, 0.65f, footprint));
...
fn fs_readability_candidate(input_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> @location(0) vec4<f32> {
    ...
    let _e24 = normal_1;
    let nDotL_3 = dot(_e24, toLight_1);
    let _e26 = normal_1;
