...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let uvCorner = vec2<f32>(select(c_2.x, -(c_2.x), (_e159 < 0.5f)), select(c_2.y, -(c_2.y), (_e174 < 0.5f)));
    ...
    let _e195 = size;
    let _e198 = p.size_rotation.w;
    let meshSize = vec3<f32>(_e195, _e198);
    let _e203 = p.size_rotation.z;
    let angles = (vec3<f32>(_e194.rotationXY, -(_e203)) * 57.29578f);
    let _e211 = dm_rotate((mv.position.xyz * meshSize), angles);
    let forward = vec3<f32>(0f, 0f, 1f);
    let _e223 = dm_rotate((mv.normal.xyz / max(abs(meshSize), vec3(0.00001f))), angles);
    let _e226 = right;
    let _e229 = up;
    let normal = normalize((((_e226 * _e223.x) + (_e229 * _e223.y)) + (forward * _e223.z)));
    let _e237 = center;
    let _e238 = right;
    let _e241 = up;
    let _e251 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e237 + ((((_e238 * _e211.x) + (_e241 * _e211.y)) + (forward * _e211.z)) * _e251));
    input.uv0_ = _e194.uv0_;
    input.custom1_ = _e194.uv1_;
    let _e262 = p.color;
    input.color = (_e262 * mv.color);
    input.normal = normal;
    let _e267 = world;
    input.worldPosition = _e267;
    let _e271 = frame.eye;
    let _e273 = world;
    input.viewDirection = (_e271.xyz - _e273);
    let _e279 = frame.settings.x;
    input.time = _e279;
    input.delta = 0.008333334f;
    let _e282 = input;
    let _e283 = source_vertex(_e282);
    let _e285 = p.age;
    let _e286 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e283, _e285);
    let _e289 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e286 + _e289.xyz);
    let _e296 = frame.vp;
    let _e297 = world;
    o.position = (_e296 * vec4<f32>(_e297, 1f));
    o.uv = _e194.uv0_;
    o.custom = _e194.uv1_;
    let _e307 = p.color;
    o.color = (_e307 * mv.color);
    let _e311 = world;
    o.world = _e311;
    let _e315 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e319 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e323 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e315.xyz, _e319.xyz, _e323.xyz) * normal));
    let _e328 = o;
    return _e328;
}
