...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    let _e83 = local_normal.x;
    let _e87 = local_normal.y;
    let _e92 = local_normal.z;
    output.normal = normalize((((input.right * _e83) + (input.up * _e87)) + (input.forward * _e92)));
    ...
    let _e109 = local_position;
    output.local_position = _e109;
    let _e110 = output;
    return _e110;
}
...
fn fs_main(input_1: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let key_light = clamp(((dot(normal, key) * 0.5f) + 0.5f), 0f, 1f);
    let rim = pow(clamp((1f - abs(dot(normal, view_proxy))), 0f, 1f), 9f);
