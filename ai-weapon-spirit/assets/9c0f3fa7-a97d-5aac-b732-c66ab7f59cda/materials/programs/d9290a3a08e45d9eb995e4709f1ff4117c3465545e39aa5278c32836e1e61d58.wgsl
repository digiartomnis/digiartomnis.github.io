...
struct CinderMeshOutX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX {
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_3: u32, count_4: u32, normal: vec3<f32>, l: vec3<f32>, useNormalBias: bool) -> f32 {
    var local: bool;
    var local_1: bool;
    var local_2: bool;
    var local_3: bool;
    ...
    let bias = select(_e33, max((_e36 * (1f - dot(normal, l))), _e46), useNormalBias);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e217 = x;
                        let _e219 = y;
                        let offsetUv = clamp((uv_1 + (vec2<f32>(f32(_e217), f32(_e219)) * texel)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e253 = x_1;
                    let _e255 = y_1;
                    let offsetUv_1 = clamp((uv_1 + (vec2<f32>(f32(_e253), f32(_e255)) * texel)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_1: vec3<f32>, worldPos_1: vec3<f32>, viewZ: f32) -> f32 {
    var shadow: f32;
    var local_5: bool;
    ...
    let _e32 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1, _e28, count_5, normal_1, l_1, true);
    shadow = _e32;
    ...
    if _e45 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t > 0f) {
                let _e69 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1, (_e28 + 1u), count_5, normal_1, l_1, true);
                shadow = mix(_e32, _e69, t);
            }
        }
    }
    let _e71 = shadow;
...
fn illustratedDiffuseX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU2LMNR2XG5DSMF2GKZC7NRUWO2DUNFXGOX(surface: vec3<f32>, shadowPigment: vec3<f32>, normal_2: vec3<f32>, visibility: f32, sideShade: f32, continuous: bool) -> vec3<f32> {
    var diffuse: f32;

    let _e3 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let nDotL_2 = dot(normal_2, normalize(-(_e3)));
    let up = clamp(normal_2.y, 0f, 1f);
    diffuse = ((0.52f * smoothstep(0f, 0.14f, nDotL_2)) + (0.48f * smoothstep(0.6f, 0.78f, nDotL_2)));
    if continuous {
        diffuse = max(0f, nDotL_2);
    }
    let fill = (mix(shadowPigment, surface, 0.16f) * vec3<f32>(0.88f, 1.02f, 1.12f));
    let ambient = (fill * mix(0.82f, 1f, up));
    let _e41 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightColor;
    let _e43 = diffuse;
    let direct = ((surface * _e41) * ((0.54f * _e43) * visibility));
    let plane = mix(sideShade, 1f, (0.65f + (0.35f * up)));
    return ((ambient + direct) * plane);
}

fn paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(base: vec3<f32>, normal_3: vec3<f32>, light: vec3<f32>, camera: vec3<f32>, roughness_2: f32, metallic: f32, strength: f32, visibility_1: f32) -> vec3<f32> {
    let halfVector = normalize((light + camera));
    let smoothness = (1f - clamp(roughness_2, 0.04f, 1f));
    let lobe = pow(max(dot(normal_3, halfVector), 0f), mix(12f, 112f, smoothness));
    let reflection = (((((strength * smoothness) * smoothness) * lobe) * max(dot(normal_3, light), 0f)) * visibility_1);
    return (mix(vec3<f32>(0.88f, 0.86f, 0.81f), base, metallic) * reflection);
}

fn cinder_printX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX(value: vec4<f32>, pixel: vec2<f32>, tint: vec4<f32>, print: vec4<f32>) -> vec4<f32> {
    var color: vec3<f32>;
...
fn cinder_hashX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX(p: vec2<f32>) -> f32 {
    return fract((sin(dot(p, vec2<f32>(127.1f, 311.7f))) * 43758.547f));
}

fn cinder_rock_shadeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX(input: CinderMeshOutX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX) -> vec4<f32> {
    var rock: vec3<f32>;

    let normal_4 = normalize(input.normal);
    ...
    let diffuse_1 = (0.18f + (0.82f * max(dot(normal_4, lightDirection), 0f)));
    let _e93 = cinder_hashX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX((floor((normal_4.xy * 19f)) + vec2(seed)));
    ...
    let rim = ((pow((1f - abs(dot(normal_4, viewDirection))), 2.2f) * charge) * charge);
    let _e147 = rock;
...
fn fs_meteor(s_1: Surface) -> @location(0) vec4<f32> {
    var source: CinderMeshOutX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX;
    ...
    let _e18 = source;
    let _e19 = cinder_rock_shadeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX(_e18);
    let _e24 = material.cinderSurface.x;
    let _e32 = material.baseColor;
    let _e35 = material.cinderPrint;
    let _e36 = cinder_printX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPXMZTYHI5GG2LOMRSXEX3NMVZWQX(vec4<f32>((_e19.xyz * _e24), 1f), s_1.clip.xy, _e32, _e35);
...
fn fs_ice(s_2: Surface) -> @location(0) vec4<f32> {
    ...
    let tip = smoothstep(0.1f, 0.82f, s_2.local.z);
    ...
    let milk = ((tip * 0.32f) + (s_2.variation * 0.16f));
    ...
    return vec4<f32>((_e50 + (_e53.xyz * (((_e2 * 0.32f) + seam) + (tip * 0.12f)))), clamp(((_e66 + (_e2 * 0.18f)) + (tip * 0.15f)), 0f, 0.92f));
}
...
fn fs_bark(s_3: Surface) -> @location(0) vec4<f32> {
    ...
    let moss = ((smoothstep(0.55f, 0.95f, sin((((s_3.local.y * 3f) + (s_3.local.x * 7f)) + (s_3.local.z * 4f)))) * smoothstep(-0.1f, 0.7f, _e1.y)) * 0.65f);
    let _e69 = material.rimColor;
    let _e72 = lit(s_3, mix(bark, _e69.xyz, moss), _e1);
...
fn fs_stratum(s_5: Surface) -> @location(0) vec4<f32> {
    ...
    let layer_4 = mix(select(0.68f, 0.92f, (strata > -0.35f)), 1f, max(0f, _e1.y));
    ...
    let _e92 = lit(s_5, ((pigment * layer_4) * (1f - (fracture * 0.24f))), _e1);
...
fn fs_blade(s_6: Surface) -> @location(0) vec4<f32> {
    let _e1 = face(s_6);
    let _e4 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v_1 = normalize((_e4 - s_6.world));
    let _e10 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let highlight = pow(max(0f, dot(reflect(-(normalize(-(_e10))), _e1), v_1)), 64f);
