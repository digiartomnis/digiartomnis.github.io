import{t as e}from"./authoring-ZeuJrv2X.js";import{o as t}from"./dist-D1HDioW-.js";import{T as n}from"./chunk-UINHLCZA-CVUXQsaP.js";import{C as r}from"./game-asset-catalog-BHEryBI7.js";import{n as i,r as a}from"./spell-solid-renderer-DDbLMrTb.js";var o={hash:`553dc4d9`,wgsl:`struct Out {
...
`},s=class{data=new Float32Array(88);bytes=new Uint8Array(this.data.buffer);published=new Float32Array(this.data.length);projection=t.create();view=t.create();vp=t.create();inverse=t.create();count=0;strength=0;sync(e,n,r,o,s,c,l=.1,u=100){let d=this.data;d.fill(0),this.count=0,this.strength=0;for(let t of e){let e=i(t),r=a(t);if(e<=0&&r<=0)continue;if(this.count===6)throw Error(`[seed-distortion] visible casts exceed equipment capacity`);let o=t.tier.seedStrike,s=16+ this.count++*12;d[s]=t.end[0],d[s+1]=t.end[1]+.15,d[s+2]=t.end[2],d[s+3]=t.tier.radiusMeters*o.distortionRadiusScale*(.35+.65*Math.min(1,t.phaseElapsed/o.sproutSeconds)),d[s+4]=n[0],d[s+5]=n[1],d[s+6]=n[2],d[s+7]=e*o.distortionPixels,d[s+8]=t.phaseElapsed,d[s+9]=t.end[1],d[s+10]=r*o.distortionPixels,d[s+11]=Math.max(0,(t.phaseElapsed-o.sproutSeconds)/o.visual.domeSeconds),this.strength=Math.max(this.strength,e,r)}this.count&&(t.perspective(this.projection,s,c,l,u),t.lookAt(this.view,n,r,o),t.multiply(this.vp,this.projection,this.view),t.invert(this.inverse,this.vp),d.set(this.inverse));let f=!1;for(let e=0;!f&&e<d.length;e++)f=d[e]!==this.published[e];return f&&this.published.set(d),f}},c=`ai-weapon-spirit::seed-distortion`,l=[0,0,0],u=[0,1,0];async function d(t,i,a){if(!i.assets||!i.renderFeatureHost)throw Error(`[seed-distortion] native host unavailable`);let d=r.parse(a);if(!d.ok)throw d.error;let f=await i.assets.loadByGuid(d.value);if(!f.ok)throw Error(`[seed-distortion] pipeline asset unavailable`,{cause:f.error});let p=f.value.config;if(f.value.kind!==`render-pipeline`||f.value.pipelineId!==`forgeax::standard`||!Array.isArray(p?.postEffects)||p.postEffects[0]!==c)throw Error(`[seed-distortion] invalid authored pipeline`);let m=new s,h=m.bytes,g=await i.renderFeatureHost.installFeature(e({identity:c,source:o.wgsl,reads:[{key:`sceneColor`},{key:`depth`,sampleType:`depth`}],params:{byteSize:h.byteLength,defaultValue:h}}));if(!g.ok)throw Error(`[seed-distortion] native feature rejected`,{cause:g.error});let _=t.spawn({component:n,data:{shader:c,data:h}});if(!_.ok)throw await g.value.release(),_.error;let v=!1,y={shader:c,data:h};return{sync(e,r,i,a,o,s,c=.1,l=100){m.sync(e,r,i,a,o,s,c,l)&&t.set(_.value,n,y).unwrap()},get strength(){return m.strength},get activeCount(){return m.count},reset(){m.sync([],l,l,u,1,1)&&t.set(_.value,n,y).unwrap()},async dispose(){if(v)return;t.despawn(_.value).unwrap();let e=await g.value.release();if(!e.ok)throw e.error;v=!0}}}export{d as t};