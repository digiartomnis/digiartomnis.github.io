...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e92 = p.position;
    let _e96 = frame.motion;
    let _e101 = frame.settings.x;
    center = (_e92.xyz + (_e96.xyz * _e101));
    let _e106 = p.size_rotation;
    size = _e106.xy;
    let _e112 = frame.layer.x;
    let columns = u32(_e112);
    let _e117 = frame.layer.y;
    let rows = u32(_e117);
    let _e121 = p.velocity.w;
    let tile = min(u32(max(_e121, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e143 = frame.extra.x;
    let _e146 = p.id;
    let _e148 = dm_random(bitcast<u32>(_e143), _e146, 210u);
    let _e158 = frame.extra.x;
    let _e161 = p.id;
    let _e163 = dm_random(bitcast<u32>(_e158), _e161, 211u);
    let uvCorner = vec2<f32>(select(c_1.x, -(c_1.x), (_e148 < 0f)), select(c_1.y, -(c_1.y), (_e163 < 0f)));
    let _e174 = frame.layer;
    let _e177 = p;
    let _e181 = frame.extra.x;
    let _e183 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e174.xy), _e177, bitcast<u32>(_e181));
    let _e186 = frame.layer;
    let _e189 = size;
    let plane = vec3<f32>(((c_1 + _e186.zw) * _e189), 0f);
    let _e196 = p.size_rotation.z;
    let _e201 = dm_rotate(plane, (vec3<f32>(_e183.rotationXY, -(_e196)) * 57.29578f));
    let _e204 = right;
    let _e205 = up;
    ...
    world = (_e208 + ((((_e209 * _e201.x) + (_e212 * _e201.y)) + (normal * _e201.z)) * _e222));
    input.uv0_ = _e183.uv0_;
    input.custom1_ = _e183.uv1_;
    let _e233 = p.color;
    input.color = _e233;
    input.normal = normal;
    let _e236 = world;
    input.worldPosition = _e236;
    let _e240 = frame.eye;
    let _e242 = world;
    input.viewDirection = (_e240.xyz - _e242);
    let _e248 = frame.settings.x;
    input.time = _e248;
    input.delta = 0.008333334f;
    let _e251 = input;
    let _e252 = source_vertex(_e251);
    let _e254 = p.age;
    let _e255 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e252, _e254);
    let _e258 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e255 + _e258.xyz);
    let _e265 = frame.vp;
    let _e266 = world;
    o.position = (_e265 * vec4<f32>(_e266, 1f));
    o.uv = _e183.uv0_;
    o.custom = _e183.uv1_;
    let _e276 = p.color;
    o.color = _e276;
    let _e278 = world;
    o.world = _e278;
    let _e282 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e286 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e290 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e282.xyz, _e286.xyz, _e290.xyz) * normal));
    let _e295 = o;
    return _e295;
}
