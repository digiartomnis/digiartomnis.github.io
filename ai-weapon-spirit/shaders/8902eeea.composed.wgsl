...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e51 = frame.invvp;
    let origin = (_e51 * vec4<f32>(native.center, 1f));
    let _e63 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    let _e66 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(((origin.xyz / vec3(origin.w)) - _e63.xyz));
    p.position = vec4<f32>(_e66, 1f);
    ...
    size = _e119.xy;
    let _e125 = frame.layer.x;
    let columns = u32(_e125);
    let _e130 = frame.layer.y;
    let rows = u32(_e130);
    let _e134 = p.velocity.w;
    let tile = min(u32(max(_e134, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e156 = frame.extra.x;
    let _e159 = p.id;
    let _e161 = dm_random(bitcast<u32>(_e156), _e159, 210u);
    let _e171 = frame.extra.x;
    let _e174 = p.id;
    let _e176 = dm_random(bitcast<u32>(_e171), _e174, 211u);
    let uvCorner = vec2<f32>(select(c_1.x, -(c_1.x), (_e161 < 0f)), select(c_1.y, -(c_1.y), (_e176 < 0f)));
    let _e187 = frame.layer;
    let _e190 = p;
    let _e194 = frame.extra.x;
    let _e196 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e187.xy), _e190, bitcast<u32>(_e194));
    let _e197 = size;
    let _e200 = p.size_rotation.w;
    let meshSize = vec3<f32>(_e197, _e200);
    let _e205 = p.size_rotation.z;
    let angles = (vec3<f32>(_e196.rotationXY, -(_e205)) * 57.29578f);
    let _e213 = dm_rotate((mv.position.xyz * meshSize), angles);
    let forward = vec3<f32>(-0.34201998f, 0f, -0.9396928f);
    let _e225 = dm_rotate((mv.normal.xyz / max(abs(meshSize), vec3(0.00001f))), angles);
    let _e228 = right;
    let _e231 = up;
    let normal = normalize((((_e228 * _e225.x) + (_e231 * _e225.y)) + (forward * _e225.z)));
    let _e239 = center;
    let _e240 = right;
    let _e243 = up;
    let _e253 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e239 + ((((_e240 * _e213.x) + (_e243 * _e213.y)) + (forward * _e213.z)) * _e253));
    input.uv0_ = _e196.uv0_;
    input.custom1_ = _e196.uv1_;
    let _e264 = p.color;
    input.color = (_e264 * mv.color);
    input.normal = normal;
    let _e269 = world;
    input.worldPosition = _e269;
    let _e273 = frame.eye;
    let _e275 = world;
    input.viewDirection = (_e273.xyz - _e275);
    let _e281 = frame.settings.x;
    input.time = _e281;
    input.delta = 0.008333334f;
    let _e284 = input;
    let _e285 = source_vertex(_e284);
    let _e287 = p.age;
    let _e288 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e285, _e287);
    let _e291 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e288 + _e291.xyz);
    let _e298 = frame.vp;
    let _e299 = world;
    o.position = (_e298 * vec4<f32>(_e299, 1f));
    o.uv = _e196.uv0_;
    o.custom = _e196.uv1_;
    let _e309 = p.color;
    o.color = (_e309 * mv.color);
    let _e313 = world;
    o.world = _e313;
    let _e317 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e321 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e325 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e317.xyz, _e321.xyz, _e325.xyz) * normal));
    let _e330 = o;
    return _e330;
}
