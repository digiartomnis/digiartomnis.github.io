...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e91 = p.position;
    let _e95 = frame.motion;
    let _e100 = frame.settings.x;
    center = (_e91.xyz + (_e95.xyz * _e100));
    let _e105 = p.size_rotation;
    size = _e105.xy;
    let _e111 = frame.layer.x;
    let columns = u32(_e111);
    let _e116 = frame.layer.y;
    let rows = u32(_e116);
    let _e120 = p.velocity.w;
    let tile = min(u32(max(_e120, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e142 = frame.extra.x;
    let _e145 = p.id;
    let _e147 = dm_random(bitcast<u32>(_e142), _e145, 210u);
    let _e157 = frame.extra.x;
    let _e160 = p.id;
    let _e162 = dm_random(bitcast<u32>(_e157), _e160, 211u);
    let uvCorner = vec2<f32>(select(c_2.x, -(c_2.x), (_e147 < 0f)), select(c_2.y, -(c_2.y), (_e162 < 0f)));
    let _e173 = frame.layer;
    let _e176 = p;
    let _e180 = frame.extra.x;
    let _e182 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e173.xy), _e176, bitcast<u32>(_e180));
    let _e185 = frame.layer;
    let _e188 = size;
    let plane = vec3<f32>(((c_2 + _e185.zw) * _e188), 0f);
    let _e195 = p.size_rotation.z;
    let _e200 = dm_rotate(plane, (vec3<f32>(_e182.rotationXY, -(_e195)) * 57.29578f));
    let _e203 = right;
    let _e204 = up;
    let normal = normalize(cross(_e203, _e204));
    let _e207 = center;
    let _e208 = right;
    let _e211 = up;
    let _e221 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e207 + ((((_e208 * _e200.x) + (_e211 * _e200.y)) + (normal * _e200.z)) * _e221));
    input.uv0_ = _e182.uv0_;
    input.custom1_ = _e182.uv1_;
    let _e232 = p.color;
    input.color = _e232;
    input.normal = normal;
    let _e235 = world;
    input.worldPosition = _e235;
    let _e239 = frame.eye;
    let _e241 = world;
    input.viewDirection = (_e239.xyz - _e241);
    let _e247 = frame.settings.x;
    input.time = _e247;
    input.delta = 0.008333334f;
    let _e250 = input;
    let _e251 = source_vertex(_e250);
    let _e253 = p.age;
    let _e254 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e251, _e253);
    let _e257 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e254 + _e257.xyz);
    let _e264 = frame.vp;
    let _e265 = world;
    o.position = (_e264 * vec4<f32>(_e265, 1f));
    o.uv = _e182.uv0_;
    o.custom = _e182.uv1_;
    let _e275 = p.color;
    o.color = _e275;
    let _e277 = world;
    o.world = _e277;
    let _e281 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e285 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e289 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e281.xyz, _e285.xyz, _e289.xyz) * normal));
    let _e294 = o;
    return _e294;
}
