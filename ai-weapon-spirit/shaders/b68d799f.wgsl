...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_14: vec3<f32>, layer_4: u32, count_4: u32, normal_14: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_2))) {
            local_24 = all((uv_5 <= (tileHi - texel_2)));
        } else {
            local_24 = false;
        }
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_26 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_26 = false;
    }
    let _e10 = local_26;
    if _e10 {
        local_27 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_27 = false;
    }
    let _e17 = local_27;
    if _e17 {
        local_28 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_28 = false;
    }
    let validReceiver = local_28;
    ...
    if all((uv_6 >= vec2(0f))) {
        local_29 = all((uv_6 <= vec2(1f)));
    } else {
        local_29 = false;
    }
    let inRange = local_29;
    if (shadowProjection.y > 0.5f) {
        local_30 = (shadowProjection.z < 0.5f);
    } else {
        local_30 = false;
    }
    let _e64 = local_30;
    if _e64 {
        local_31 = (shadowProjection.x > 0f);
    } else {
        local_31 = false;
    }
    let available = local_31;
