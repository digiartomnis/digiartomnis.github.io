...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    f0_ = mix(((vec3(dielectricF0_) * _e55) * _e57), albedo_2, metallic_13);
    physicalNormal = n_5;
    ...
    let kD_1 = ((vec3(1f) - _e70) * (1f - metallic_13));
    ...
    let _e200 = coatF;
    ambient = (((((kD_1 * _e189) * _e191) * skyFactor) + (_e194 * _e188)) * (vec3(1f) - _e200));
    ...
    let _e231 = probeBlendRecords[(probeBase + 1u)];
    let _e236 = probeBlendRecords[(probeBase + 2u)];
    let _e241 = probeBlendRecords[(probeBase + 3u)];
    let _e246 = probeBlendRecords[(probeBase + 4u)];
    let _e251 = probeBlendRecords[(probeBase + 5u)];
    let _e256 = probeBlendRecords[(probeBase + 6u)];
    let _e261 = probeBlendRecords[(probeBase + 7u)];
    let _e266 = probeBlendRecords[(probeBase + 8u)];
    let _e271 = probeBlendRecords[(probeBase + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e231, _e236, _e241, _e246, _e251, _e256, _e261, _e266, _e271);
    let probeLocalBlendFraction = probeBlendRecords[probeBase].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_13)), vec3(0.0001f)));
    let _e284 = physicalNormal;
    let _e285 = irradiance;
    let _e287 = diffuseAlbedo;
    let _e288 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e284, (_e285 * skyFactor), probeDiffuseK, _e287, metallic_13);
    if probeIdentityValid {
        let _e289 = specularIbl;
        let _e294 = coatF;
        ambient = ((_e288 + (_e289 * _e188)) * (vec3(1f) - _e294));
    }
    let _e297 = ambient;
    ambient = (_e297 * ao);
    let _e299 = ambient;
    let _e303 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e306 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e303, ssaoBlurredTexture, ssaoBlurredSampler);
    ambient = (_e299 * _e306);
    let _e308 = ambient;
    color = _e308;
    let _e310 = color;
    color = (_e310 + _e9.emissive);
    let _e313 = physicalNormal;
    let _e315 = standardViewZ(in_2);
    let _e316 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e313, in_2.worldPos, _e315);
    let _e317 = physicalNormal;
    let _e318 = diffuseAlbedo;
    let _e319 = f0_;
    let _e320 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e317, v_5, _e318, metallic_13, a_3, _e319);
    let _e321 = color;
    let _e326 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e330 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e334 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e338 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e339 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e316 * _e320), in_2.worldPos, _e326.xyz, _e330.xyz, _e334.xyz, _e338);
    color = (_e321 + _e339);
    let _e341 = color;
    let _e344 = standardViewZ(in_2);
    let _e346 = physicalNormal;
    let _e347 = diffuseAlbedo;
    let _e348 = f0_;
    let _e351 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e344, in_2.worldPos, _e346, v_5, _e347, metallic_13, a_3, _e348, false, true);
    color = (_e341 + _e351);
    let _e353 = color;
    return vec4<f32>(_e353, alpha);
}
