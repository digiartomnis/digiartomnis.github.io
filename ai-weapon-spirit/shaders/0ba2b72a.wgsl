...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e352 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e348, (_e349 * skyFactor), probeDiffuseK, _e351, metallic_4);
    if true {
    ...
    let _e437 = material.thickness;
    let _e439 = finiteScalar(_e437, 0f);
    let _e442 = thicknessSample;
    let _e444 = finiteScalar(_e442, 1f);
    let worldThickness = max(((_e439 * length(worldRefractedDirection)) * _e444), 0f);
    ...
    let _e486 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e486.xyz;
    ...
    let _e494 = local_1;
    if _e494 {
        let _e502 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e502.xyz;
    }
    let _e506 = material.attenuationColor;
    let _e509 = finiteColor(_e506, vec3(1f));
    let safeAttenuationColor = clamp(_e509, vec3(0f), vec3(1f));
    let _e517 = material.attenuationDistance;
    let _e519 = finiteScalar(_e517, 0f);
    let safeAttenuationDistance = max(_e519, 0f);
    ...
    let _e536 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e536);
    let _e538 = color;
    let _e539 = transmittedBackdrop;
    let _e542 = finiteColor(_e539, vec3(0f));
    color = (_e538 + ((_e542 * transmittedEnergy) * beerAttenuation));
    let _e546 = color;
    color = (_e546 + _e7.emissive);
    ...
    let _e555 = f0_;
    let _e556 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e553, v, _e554, metallic_4, a_2, _e555);
    let _e557 = color;
