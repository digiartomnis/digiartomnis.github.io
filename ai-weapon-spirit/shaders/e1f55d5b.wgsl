...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_5: u32, count_4: u32, normal_8: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    let _e60 = local_5;
    if _e60 {
        local_6 = (projCoords.z >= 0f);
    } else {
        local_6 = false;
    }
    let _e67 = local_6;
    if _e67 {
        local_7 = (projCoords.z <= 1f);
    } else {
        local_7 = false;
    }
    let _e74 = local_7;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_11 = (worldUnitsPerTexel > 0f);
    } else {
        local_11 = false;
    }
    let _e213 = local_11;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_6: u32, count_5: u32, normal_9: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_14 = (tileUv_1.x <= 1f);
    } else {
        local_14 = false;
    }
    let _e40 = local_14;
    if _e40 {
        local_15 = (tileUv_1.y >= 0f);
    } else {
        local_15 = false;
    }
    let _e47 = local_15;
    if _e47 {
        local_16 = (tileUv_1.y <= 1f);
    } else {
        local_16 = false;
    }
    let _e54 = local_16;
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel_3))) {
            local_18 = all((uv_4 <= (tileHi - texel_3)));
        } else {
            local_18 = false;
        }
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_20 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_20 = false;
    }
    let _e10 = local_20;
    if _e10 {
        local_21 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_21 = false;
    }
    let _e17 = local_21;
    if _e17 {
        local_22 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_22 = false;
    }
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let metallic_5 = clamp(_e14, 0f, 1f);
    ...
    let _e55 = f0_;
    let _e56 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e51, v), 0f), _e55, iblRoughness);
    let kD_1 = ((vec3(1f) - _e56) * (1f - metallic_5));
    ...
    if (_e68 < 0f) {
        ...
        let _e145 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e136, v, iblRoughness, _e137, _e140, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e145;
    }
    ...
    let _e295 = ambient;
    color = _e295;
    let _e297 = physicalNormal;
    let _e299 = standardViewZ(in_2);
    let _e300 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e297, in_2.worldPos, _e299);
    let _e301 = physicalNormal;
    let _e302 = diffuseAlbedo;
    let _e303 = f0_;
    let _e304 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e301, v, _e302, metallic_5, a_3, _e303);
    let _e305 = color;
    let _e310 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e314 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e318 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e322 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e323 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e300 * _e304), in_2.worldPos, _e310.xyz, _e314.xyz, _e318.xyz, _e322);
    color = (_e305 + _e323);
    let _e325 = color;
    color = (_e325 + _e9.emissive);
    let _e330 = color;
    output.color = vec4<f32>(_e330, alpha_1);
    let _e333 = reflectionFallback;
    let _e334 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e333, _e334);
    let _e336 = output;
    return _e336;
}
