...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_23 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_23 = false;
    }
    let _e10 = local_23;
    if _e10 {
        local_24 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_24 = false;
    }
    let _e17 = local_24;
    if _e17 {
        local_25 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_25 = false;
    }
...
fn composeProbeDiffuse(shPreblend_2: array<vec4<f32>, 9>, localBlendFraction_1: f32, normal_12: vec3<f32>, skyIrradiance: vec3<f32>, kD: vec3<f32>, albedo_1: vec3<f32>, metallic_2: f32) -> vec3<f32> {
    let _e7 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_2, localBlendFraction_1, normal_12, skyIrradiance, kD, albedo_1, metallic_2);
    return _e7;
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e137 = f0_;
    let _e138 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e133, v), 0f), _e137, iblRoughness);
    ...
    if (_e150 < 0f) {
        ...
        let _e227 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e218, v, iblRoughness, _e219, _e222, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e227;
    }
    ...
    let _e363 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e359, (_e360 * skyFactor), probeDiffuseK, _e362, metallic_5);
    ...
    let _e450 = material.thickness;
    let _e452 = finiteScalar(_e450, 0f);
    let _e455 = thicknessSample;
    let _e457 = finiteScalar(_e455, 1f);
    let worldThickness = max(((_e452 * length(worldRefractedDirection)) * _e457), 0f);
    ...
    let _e499 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e499.xyz;
    ...
    let _e507 = local_2;
    if _e507 {
        let _e515 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e515.xyz;
    }
    let _e519 = material.attenuationColor;
    let _e522 = finiteColor(_e519, vec3(1f));
    let safeAttenuationColor = clamp(_e522, vec3(0f), vec3(1f));
    let _e530 = material.attenuationDistance;
    let _e532 = finiteScalar(_e530, 0f);
    let safeAttenuationDistance = max(_e532, 0f);
    ...
    let _e549 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e549);
    let _e551 = color;
    let _e552 = transmittedBackdrop;
    let _e555 = finiteColor(_e552, vec3(0f));
    color = (_e551 + ((_e555 * transmittedEnergy) * beerAttenuation));
    let _e559 = physicalNormal;
    let _e561 = standardViewZ(in_2);
    let _e562 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e559, in_2.worldPos, _e561);
    let _e563 = physicalNormal;
    let _e564 = diffuseAlbedo;
    let _e565 = f0_;
    let _e566 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e563, v, _e564, metallic_5, a_3, _e565);
    let _e567 = color;
    let _e572 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e576 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e580 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e584 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e585 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e562 * _e566), in_2.worldPos, _e572.xyz, _e576.xyz, _e580.xyz, _e584);
    color = (_e567 + _e585);
    let _e587 = color;
    color = (_e587 + _e7.emissive);
    let _e592 = color;
    output.color = vec4<f32>(_e592, alpha_1);
    let _e595 = reflectionFallback;
    let _e596 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e595, _e596);
    let _e598 = output;
    return _e598;
}
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_1, normalize((_e28 - in_3.worldPos)), roughness_8, _e32, brdfLut_3, irradianceSampler_1);
