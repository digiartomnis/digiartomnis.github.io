...
// ForgeAX renders reversed-Z (near 1, far and the clear value 0), matching
// mat4.perspectiveReverseZ in every camera matrix packed for this shader.
fn linearDepth(ndcDepth : f32) -> f32 {
  let nearPlane = p.depth.x;
  let farPlane = p.depth.y;
  return nearPlane * farPlane
    / max(0.00001, nearPlane + ndcDepth * (farPlane - nearPlane));
}
...
fn waterReflection(scene : vec3<f32>, uv : vec2<f32>, rawDepth : f32,
  ...
  for (var i = 0u; i < 40u; i += 1u) {
    ...
    if (depth > .000001 && sampleWorld.y > p.water.x + p.caseStyle.w && clip.w >= linearDepth(depth)) {
