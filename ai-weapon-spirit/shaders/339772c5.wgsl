...
fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_7: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_8: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_7 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_7 = true;
        }
        let _e90 = local_7;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_8 = (cookieUv.x > 1f);
            } else {
                local_8 = true;
            }
            let _e137 = local_8;
            if !(_e137) {
                local_9 = (cookieUv.y < 0f);
            } else {
                local_9 = true;
            }
            let _e145 = local_9;
            if !(_e145) {
                local_10 = (cookieUv.y > 1f);
            } else {
                local_10 = true;
            }
            let _e153 = local_10;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let alpha_1 = _e9.opacity;
    let albedo_3 = _e9.baseColor;
    let _e14 = finiteScalar(_e9.metallic, 0f);
    let metallic_14 = clamp(_e14, 0f, 1f);
    ...
    let _e35 = finiteScalar(_e7.ior, 1.5f);
    let safeIor_1 = max(_e35, 1f);
    let dielectricF0_1 = pow(((safeIor_1 - 1f) / (safeIor_1 + 1f)), 2f);
    let _e45 = standardSurfaceF0_(in_2, _e9);
    f0_ = _e45;
    physicalNormal = n_5;
    diffuseAlbedo = albedo_3;
    let _e51 = physicalNormal;
    let _e55 = f0_;
    let _e56 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e51, v_5), 0f), _e55, iblRoughness);
    let kD_1 = ((vec3(1f) - _e56) * (1f - metallic_14));
    let _e62 = coatRoughness;
    let _e63 = coatRoughness;
    coatAlpha = (_e62 * _e63);
    let _e68 = skylight.intensity;
    if (_e68 < 0f) {
        let _e71 = physicalNormal;
        let _e74 = skylight.diffuseRotation;
        let _e77 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e71, _e74, irradianceMap_1, irradianceSampler_1);
        irradiance = _e77;
        let _e79 = physicalNormal;
        let _e80 = f0_;
        let _e84 = skylight.colorR;
        let _e87 = skylight.colorG;
        let _e90 = skylight.colorB;
        let _e94 = skylight.rotation;
        let _e103 = skylight.diffuseRotation;
        let _e106 = skylight.diffuseScale;
        let _e110 = skylight.intensity;
        let _e119 = skylight.rotation.w;
        let _e127 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e79, v_5, iblRoughness, _e80, in_2.worldPos, vec3<f32>(_e84, _e87, _e90), _e94.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1, skylightPrefilterMap, _e103, _e106.xyz, max((-(_e110) - 1f), 0f), (_e119 > 0.5f));
        specularIbl = _e127;
    } else {
        let _e129 = physicalNormal;
        let _e132 = skylight.rotation;
        let _e135 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e129, _e132, irradianceMap_1, irradianceSampler_1);
        irradiance = _e135;
        let _e136 = physicalNormal;
        let _e137 = f0_;
        let _e140 = skylight.rotation;
        let _e145 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e136, v_5, iblRoughness, _e137, _e140, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e145;
    }
    let ao = _e9.occlusion;
    let _e149 = skylight.colorR;
    let _e152 = skylight.colorG;
    let _e155 = skylight.colorB;
    let skyColor_1 = vec3<f32>(_e149, _e152, _e155);
    let _e159 = skylight.intensity;
    let _e163 = skylight.diffuseScale;
    let _e167 = skylight.intensity;
    let skyFactor = select((skyColor_1 * _e159), _e163.xyz, (_e167 < 0f));
    let _e173 = skylight.intensity;
    let _e174 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(skyColor_1, _e173);
    let _e175 = specularIbl;
    let _e179 = coatF;
    reflectionFallback = (_e175 * (vec3(1f) - _e179));
    let _e183 = irradiance;
    let _e185 = diffuseAlbedo;
    let _e188 = specularIbl;
    let _e193 = coatF;
    ambient = (((((kD_1 * _e183) * _e185) * skyFactor) + (_e188 * _e174)) * (vec3(1f) - _e193));
    ...
    let _e224 = probeBlendRecords[(probeBase + 1u)];
    let _e229 = probeBlendRecords[(probeBase + 2u)];
    let _e234 = probeBlendRecords[(probeBase + 3u)];
    let _e239 = probeBlendRecords[(probeBase + 4u)];
    let _e244 = probeBlendRecords[(probeBase + 5u)];
    let _e249 = probeBlendRecords[(probeBase + 6u)];
    let _e254 = probeBlendRecords[(probeBase + 7u)];
    let _e259 = probeBlendRecords[(probeBase + 8u)];
    let _e264 = probeBlendRecords[(probeBase + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e224, _e229, _e234, _e239, _e244, _e249, _e254, _e259, _e264);
    ...
    if probeIdentityValid {
        ...
        let _e287 = coatF;
        ambient = ((_e281 + (_e282 * _e174)) * (vec3(1f) - _e287));
    }
    let _e290 = ambient;
    ambient = (_e290 * ao);
    let _e292 = reflectionFallback;
    reflectionFallback = ((_e292 * _e174) * ao);
    let _e298 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e301 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e298, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e302 = ambient;
    ambient = (_e302 * _e301);
    let _e304 = reflectionFallback;
    reflectionFallback = (_e304 * _e301);
    let _e306 = ambient;
    color = _e306;
    let _e308 = physicalNormal;
    let _e310 = standardViewZ(in_2);
    let _e311 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e308, in_2.worldPos, _e310);
    let _e312 = physicalNormal;
    let _e313 = diffuseAlbedo;
    let _e314 = f0_;
    let _e315 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e312, v_5, _e313, metallic_14, a_4, _e314);
    let _e316 = color;
    let _e321 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e325 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e329 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e333 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e334 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e311 * _e315), in_2.worldPos, _e321.xyz, _e325.xyz, _e329.xyz, _e333);
    color = (_e316 + _e334);
    let _e336 = color;
    let _e339 = standardViewZ(in_2);
    let _e341 = physicalNormal;
    let _e342 = diffuseAlbedo;
    let _e343 = f0_;
    let _e346 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e339, in_2.worldPos, _e341, v_5, _e342, metallic_14, a_4, _e343, false, true);
    color = (_e336 + _e346);
    let _e348 = color;
    color = (_e348 + _e9.emissive);
    let _e353 = color;
    output.color = vec4<f32>(_e353, alpha_1);
    let _e356 = reflectionFallback;
    let _e357 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e356, _e357);
    let _e359 = output;
    return _e359;
}
