...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_1: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_42 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_42 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn composeProbeDiffuse(shPreblend_2: array<vec4<f32>, 9>, localBlendFraction_1: f32, normal_18: vec3<f32>, skyIrradiance: vec3<f32>, kD: vec3<f32>, albedo_1: vec3<f32>, metallic_10: f32) -> vec3<f32> {
    let _e7 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_2, localBlendFraction_1, normal_18, skyIrradiance, kD, albedo_1, metallic_10);
    return _e7;
}
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e137 = f0_;
    let _e138 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e133, v_2), 0f), _e137, iblRoughness);
    ...
    if (_e150 < 0f) {
        ...
        let _e227 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e218, v_2, iblRoughness, _e219, _e222, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e227;
    }
    ...
    let _e284 = probeBlendRecords[(0u + 1u)];
    let _e289 = probeBlendRecords[(0u + 2u)];
    let _e294 = probeBlendRecords[(0u + 3u)];
    let _e299 = probeBlendRecords[(0u + 4u)];
    let _e304 = probeBlendRecords[(0u + 5u)];
    let _e309 = probeBlendRecords[(0u + 6u)];
    let _e314 = probeBlendRecords[(0u + 7u)];
    let _e319 = probeBlendRecords[(0u + 8u)];
    let _e324 = probeBlendRecords[(0u + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e284, _e289, _e294, _e299, _e304, _e309, _e314, _e319, _e324);
    let probeLocalBlendFraction = probeBlendRecords[0u].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_13)), vec3(0.0001f)));
    let _e337 = physicalNormal;
    let _e338 = irradiance;
    let _e340 = diffuseAlbedo;
    let _e341 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e337, (_e338 * skyFactor), probeDiffuseK, _e340, metallic_13);
    if true {
        let _e343 = specularIbl;
        let _e348 = coatF;
        ambient = ((_e341 + (_e343 * _e256)) * (vec3(1f) - _e348));
    }
    let _e351 = ambient;
    ambient = (_e351 * ao);
    let _e353 = reflectionFallback;
    reflectionFallback = ((_e353 * _e256) * ao);
    let _e359 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e362 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e359, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e363 = ambient;
    ambient = (_e363 * _e362);
    let _e365 = reflectionFallback;
    reflectionFallback = (_e365 * _e362);
    let _e367 = ambient;
    color = _e367;
    ...
    let _e440 = material.thickness;
    let _e442 = finiteScalar(_e440, 0f);
    let _e445 = thicknessSample;
    let _e447 = finiteScalar(_e445, 1f);
    let worldThickness = max(((_e442 * length(worldRefractedDirection)) * _e447), 0f);
    ...
    let _e489 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e489.xyz;
    ...
    let _e497 = local_1;
    if _e497 {
        let _e505 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e505.xyz;
    }
    let _e509 = material.attenuationColor;
    let _e512 = finiteColor(_e509, vec3(1f));
    let safeAttenuationColor = clamp(_e512, vec3(0f), vec3(1f));
    let _e520 = material.attenuationDistance;
    let _e522 = finiteScalar(_e520, 0f);
    let safeAttenuationDistance = max(_e522, 0f);
    ...
    let _e539 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e539);
    let _e541 = color;
    let _e542 = transmittedBackdrop;
    let _e545 = finiteColor(_e542, vec3(0f));
    color = (_e541 + ((_e545 * transmittedEnergy) * beerAttenuation));
    let _e549 = physicalNormal;
    let _e551 = standardViewZ(in_1);
    let _e552 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e549, in_1.worldPos, _e551);
    let _e553 = physicalNormal;
    let _e554 = diffuseAlbedo;
    let _e555 = f0_;
    let _e556 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e553, v_2, _e554, metallic_13, a_3, _e555);
    let _e557 = color;
    let _e562 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e566 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e570 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e574 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e575 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e552 * _e556), in_1.worldPos, _e562.xyz, _e566.xyz, _e570.xyz, _e574);
    color = (_e557 + _e575);
    let _e577 = color;
    let _e580 = standardViewZ(in_1);
    let _e582 = physicalNormal;
    let _e583 = diffuseAlbedo;
    let _e584 = f0_;
    let _e587 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e580, in_1.worldPos, _e582, v_2, _e583, metallic_13, a_3, _e584, false, true);
    color = (_e577 + _e587);
    let _e589 = color;
    color = (_e589 + _e7.emissive);
    let _e594 = color;
    output.color = vec4<f32>(_e594, alpha_1);
    let _e597 = reflectionFallback;
    let _e598 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e597, _e598);
    let _e600 = output;
    return _e600;
}
...
fn fs_gbuffer(in_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_3, normalize((_e28 - in_2.worldPos)), roughness_9, _e32, brdfLut_3, irradianceSampler_1);
