...
fn vs_main_impl(in_6: VsIn, meshIndex: u32, instanceIndex: u32, paletteBase: u32, materialIndex_1: u32, materialAddress: vec3<u32>) -> VsOut {
    ...
    out_2.positionOSAndViewZ = vec4<f32>(in_6.pos, _e75);
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e7 = selectedMaterial(in_2.materialAddress.x);
    let _e9 = evaluateStandardSurface(in_2, frontFacing);
    alphaTestSurface(_e9);
    let alpha = _e9.opacity;
    let albedo = _e9.baseColor;
    let _e14 = finiteScalar(_e9.metallic, 0f);
    let metallic_11 = clamp(_e14, 0f, 1f);
    let _e20 = finiteScalar(_e9.roughness, 0.5f);
    let iblRoughness = clamp(_e20, 0.04f, 1f);
    let a_3 = (iblRoughness * iblRoughness);
    let n_5 = normalize(_e9.normalWS);
    let _e29 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v_5 = normalize((_e29 - in_2.worldPos));
    specularColor = _e7.specularColor;
    let _e37 = finiteScalar(_e7.specular, 1f);
    specularWeight = clamp(_e37, 0f, 1f);
    let _e44 = finiteScalar(_e7.ior, 1.5f);
    let safeIor = max(_e44, 1f);
    let dielectricF0_ = pow(((safeIor - 1f) / (safeIor + 1f)), 2f);
    let _e55 = specularColor;
    let _e57 = specularWeight;
    f0_ = mix(((vec3(dielectricF0_) * _e55) * _e57), albedo, metallic_11);
    physicalNormal = n_5;
    diffuseAlbedo = albedo;
    let _e65 = physicalNormal;
    let _e69 = f0_;
    let _e70 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e65, v_5), 0f), _e69, iblRoughness);
    let kD = ((vec3(1f) - _e70) * (1f - metallic_11));
    let _e76 = coatRoughness;
    let _e77 = coatRoughness;
    coatAlpha = (_e76 * _e77);
    let _e82 = skylight.intensity;
    if (_e82 < 0f) {
        let _e85 = physicalNormal;
        let _e88 = skylight.diffuseRotation;
        let _e91 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e85, _e88, irradianceMap_1, irradianceSampler_1);
        irradiance = _e91;
        let _e93 = physicalNormal;
        let _e94 = f0_;
        let _e98 = skylight.colorR;
        let _e101 = skylight.colorG;
        let _e104 = skylight.colorB;
        let _e108 = skylight.rotation;
        let _e117 = skylight.diffuseRotation;
        let _e120 = skylight.diffuseScale;
        let _e124 = skylight.intensity;
        let _e133 = skylight.rotation.w;
        let _e141 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e93, v_5, iblRoughness, _e94, in_2.worldPos, vec3<f32>(_e98, _e101, _e104), _e108.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1, skylightPrefilterMap, _e117, _e120.xyz, max((-(_e124) - 1f), 0f), (_e133 > 0.5f));
        specularIbl = _e141;
    } else {
        let _e143 = physicalNormal;
        let _e146 = skylight.rotation;
        let _e149 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e143, _e146, irradianceMap_1, irradianceSampler_1);
        irradiance = _e149;
        let _e150 = physicalNormal;
        let _e151 = f0_;
        let _e154 = skylight.rotation;
        let _e159 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e150, v_5, iblRoughness, _e151, _e154, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e159;
    }
    let ao = _e9.occlusion;
    let _e163 = skylight.colorR;
    let _e166 = skylight.colorG;
    let _e169 = skylight.colorB;
    let skyColor_1 = vec3<f32>(_e163, _e166, _e169);
    let _e173 = skylight.intensity;
    let _e177 = skylight.diffuseScale;
    let _e181 = skylight.intensity;
    let skyFactor = select((skyColor_1 * _e173), _e177.xyz, (_e181 < 0f));
    let _e187 = skylight.intensity;
    let _e188 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(skyColor_1, _e187);
    let _e189 = irradiance;
    let _e191 = diffuseAlbedo;
    let _e194 = specularIbl;
    let _e200 = coatF;
    ambient = (((((kD * _e189) * _e191) * skyFactor) + (_e194 * _e188)) * (vec3(1f) - _e200));
    let _e204 = ambient;
    ambient = (_e204 * ao);
    let _e206 = ambient;
    let _e210 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e213 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e210, ssaoBlurredTexture, ssaoBlurredSampler);
    ambient = (_e206 * _e213);
    let _e215 = ambient;
    color = _e215;
    let _e217 = color;
    color = (_e217 + _e9.emissive);
    let _e220 = physicalNormal;
    let _e222 = standardViewZ(in_2);
    let _e223 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e220, in_2.worldPos, _e222);
    let _e224 = physicalNormal;
    let _e225 = diffuseAlbedo;
    let _e226 = f0_;
    let _e227 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e224, v_5, _e225, metallic_11, a_3, _e226);
    let _e228 = color;
    let _e233 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e237 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e241 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e245 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e246 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e223 * _e227), in_2.worldPos, _e233.xyz, _e237.xyz, _e241.xyz, _e245);
    color = (_e228 + _e246);
    let _e248 = color;
    let _e251 = standardViewZ(in_2);
    let _e253 = physicalNormal;
    let _e254 = diffuseAlbedo;
    let _e255 = f0_;
    let _e258 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e251, in_2.worldPos, _e253, v_5, _e254, metallic_11, a_3, _e255, false, true);
    color = (_e248 + _e258);
    let _e260 = color;
    return vec4<f32>(_e260, alpha);
}
