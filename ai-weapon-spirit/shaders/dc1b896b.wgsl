...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_22 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_22 = false;
    }
    let _e10 = local_22;
    if _e10 {
        local_23 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_23 = false;
    }
    let _e17 = local_23;
    if _e17 {
        local_24 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_24 = false;
    }
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e341 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e337, (_e338 * skyFactor), probeDiffuseK, _e340, metallic_5);
    if true {
    ...
    let _e429 = material.thickness;
    let _e431 = finiteScalar(_e429, 0f);
    let _e434 = thicknessSample;
    let _e436 = finiteScalar(_e434, 1f);
    let worldThickness = max(((_e431 * length(worldRefractedDirection)) * _e436), 0f);
    ...
    let _e478 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e478.xyz;
    ...
    let _e486 = local_1;
    if _e486 {
        let _e494 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e494.xyz;
    }
    let _e498 = material.attenuationColor;
    let _e501 = finiteColor(_e498, vec3(1f));
    let safeAttenuationColor = clamp(_e501, vec3(0f), vec3(1f));
    let _e509 = material.attenuationDistance;
    let _e511 = finiteScalar(_e509, 0f);
    let safeAttenuationDistance = max(_e511, 0f);
    ...
    let _e528 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e528);
    let _e530 = color;
    let _e531 = transmittedBackdrop;
    let _e534 = finiteColor(_e531, vec3(0f));
    color = (_e530 + ((_e534 * transmittedEnergy) * beerAttenuation));
    let _e538 = physicalNormal;
    let _e540 = standardViewZ(in_1);
    let _e541 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e538, in_1.worldPos, _e540);
    let _e542 = physicalNormal;
    let _e543 = diffuseAlbedo;
    let _e544 = f0_;
    let _e545 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e542, v, _e543, metallic_5, a_3, _e544);
    let _e546 = color;
    let _e551 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e555 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e559 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e563 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e564 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e541 * _e545), in_1.worldPos, _e551.xyz, _e555.xyz, _e559.xyz, _e563);
    color = (_e546 + _e564);
    let _e566 = color;
    color = (_e566 + _e7.emissive);
    let _e571 = color;
    output.color = vec4<f32>(_e571, alpha_1);
    let _e574 = reflectionFallback;
    let _e575 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e574, _e575);
    let _e577 = output;
    return _e577;
}
...
fn fs_gbuffer(in_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_1, normalize((_e28 - in_2.worldPos)), roughness_8, _e32, brdfLut_3, irradianceSampler_1);
