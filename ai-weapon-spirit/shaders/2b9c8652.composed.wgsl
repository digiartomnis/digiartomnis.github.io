...
struct NativeInput {
    ...
    @location(13) uv1_: vec2<f32>,
    @location(14) particle_velocity: vec4<f32>,
    @location(15) particle_size: vec4<f32>,
}
...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e45 = frame.invvp;
    let origin = (_e45 * vec4<f32>(native.center, 1f));
    ...
    size = _e101.xy;
    let _e106 = frame.right;
    let _e108 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e106.xyz);
    right = _e108;
    let _e112 = frame.up;
    let _e114 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e112.xyz);
    up = _e114;
    let _e117 = p.velocity;
    let speed = length(_e117.xyz);
    let _e125 = p.velocity;
    up = select(vec3<f32>(-0.00000000000000011427388f, 0.00000000000000000000000000000005517193f, 1f), (_e125.xyz / vec3(max(speed, 0.000001f))), (speed > 0.00001f));
    let _e134 = up;
    let _e137 = frame.eye;
    let _e141 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    let _e144 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX((_e137.xyz - _e141.xyz));
    let _e145 = center;
    let across = cross(_e134, (_e144 - _e145));
    let _e150 = frame.right;
    let _e152 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e150.xyz);
    right = select(_e152, (across / vec3(max(length(across), 0.000001f))), (length(across) > 0.00001f));
    let _e164 = size.y;
    size.y = ((_e164 * 0f) + (speed * 0.5f));
    let _e173 = frame.layer.x;
    let columns = u32(_e173);
    let _e178 = frame.layer.y;
    let rows = u32(_e178);
    let _e182 = p.velocity.w;
    let tile = min(u32(max(_e182, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e204 = frame.extra.x;
    let _e207 = p.id;
    let _e209 = dm_random(bitcast<u32>(_e204), _e207, 210u);
    let _e219 = frame.extra.x;
    let _e222 = p.id;
    let _e224 = dm_random(bitcast<u32>(_e219), _e222, 211u);
    let uvCorner = vec2<f32>(select(c_1.x, -(c_1.x), (_e209 < 0f)), select(c_1.y, -(c_1.y), (_e224 < 0f)));
    let _e235 = frame.layer;
    let _e238 = p;
    let _e242 = frame.extra.x;
    let _e244 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e235.xy), _e238, bitcast<u32>(_e242));
    let _e247 = frame.layer;
    let _e250 = size;
    let plane = vec3<f32>(((c_1 + _e247.zw) * _e250), 0f);
    let _e257 = p.size_rotation.z;
    let _e262 = dm_rotate(plane, (vec3<f32>(_e244.rotationXY, -(_e257)) * 57.29578f));
    let _e263 = right;
    let _e264 = up;
    let normal = normalize(cross(_e263, _e264));
    let _e267 = center;
    let _e268 = right;
    let _e271 = up;
    let _e281 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e267 + ((((_e268 * _e262.x) + (_e271 * _e262.y)) + (normal * _e262.z)) * _e281));
    input.uv0_ = _e244.uv0_;
    input.custom1_ = _e244.uv1_;
    let _e292 = p.color;
    input.color = _e292;
    input.normal = normal;
    let _e295 = world;
    input.worldPosition = _e295;
    let _e299 = frame.eye;
    let _e301 = world;
    input.viewDirection = (_e299.xyz - _e301);
    let _e307 = frame.settings.x;
    input.time = _e307;
    input.delta = 0.008333334f;
    let _e310 = input;
    let _e311 = source_vertex(_e310);
    let _e313 = p.age;
    let _e314 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e311, _e313);
    let _e317 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e314 + _e317.xyz);
    let _e324 = frame.vp;
    let _e325 = world;
    o.position = (_e324 * vec4<f32>(_e325, 1f));
    o.uv = _e244.uv0_;
    o.custom = _e244.uv1_;
    let _e335 = p.color;
    o.color = _e335;
    let _e337 = world;
    o.world = _e337;
    let _e341 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e345 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e349 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e341.xyz, _e345.xyz, _e349.xyz) * normal));
    let _e354 = o;
    return _e354;
}
