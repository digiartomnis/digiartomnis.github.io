...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_21 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_21 = false;
    }
    let _e10 = local_21;
    if _e10 {
        local_22 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_22 = false;
    }
    let _e17 = local_22;
    if _e17 {
        local_23 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_23 = false;
    }
    let validReceiver = local_23;
    ...
    if all((uv_6 >= vec2(0f))) {
        local_24 = all((uv_6 <= vec2(1f)));
    } else {
        local_24 = false;
    }
    let inRange = local_24;
    if (shadowProjection.y > 0.5f) {
        local_25 = (shadowProjection.z < 0.5f);
    } else {
        local_25 = false;
    }
    let _e64 = local_25;
    if _e64 {
        local_26 = (shadowProjection.x > 0f);
    } else {
        local_26 = false;
    }
    let available = local_26;
