...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e54 = f0_;
    let _e55 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e50, v_5), 0f), _e54, iblRoughness);
    ...
    if (_e67 < 0f) {
        ...
        let _e73 = skylight.diffuseRotation;
        let _e76 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e70, _e73, irradianceMap_1, irradianceSampler_1);
        irradiance = _e76;
        let _e78 = physicalNormal;
        let _e79 = f0_;
        let _e83 = skylight.colorR;
        let _e86 = skylight.colorG;
        let _e89 = skylight.colorB;
        let _e93 = skylight.rotation;
        let _e102 = skylight.diffuseRotation;
        let _e105 = skylight.diffuseScale;
        let _e109 = skylight.intensity;
        let _e118 = skylight.rotation.w;
        let _e126 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e78, v_5, iblRoughness, _e79, in_1.worldPos, vec3<f32>(_e83, _e86, _e89), _e93.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1, skylightPrefilterMap, _e102, _e105.xyz, max((-(_e109) - 1f), 0f), (_e118 > 0.5f));
        specularIbl = _e126;
    } else {
        let _e128 = physicalNormal;
        let _e131 = skylight.rotation;
        let _e134 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e128, _e131, irradianceMap_1, irradianceSampler_1);
        irradiance = _e134;
        let _e135 = physicalNormal;
        let _e136 = f0_;
        let _e139 = skylight.rotation;
        let _e144 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e135, v_5, iblRoughness, _e136, _e139, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e144;
    }
    let ao = _e6.occlusion;
    let _e148 = skylight.colorR;
    let _e151 = skylight.colorG;
    let _e154 = skylight.colorB;
    let skyColor_1 = vec3<f32>(_e148, _e151, _e154);
    let _e158 = skylight.intensity;
    let _e162 = skylight.diffuseScale;
    let _e166 = skylight.intensity;
    let skyFactor = select((skyColor_1 * _e158), _e162.xyz, (_e166 < 0f));
    let _e172 = skylight.intensity;
    let _e173 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(skyColor_1, _e172);
    let _e174 = specularIbl;
    let _e178 = coatF;
    reflectionFallback = (_e174 * (vec3(1f) - _e178));
    let _e182 = irradiance;
    let _e184 = diffuseAlbedo;
    let _e187 = specularIbl;
    let _e192 = coatF;
    ambient = (((((kD_1 * _e182) * _e184) * skyFactor) + (_e187 * _e173)) * (vec3(1f) - _e192));
    let _e201 = probeBlendRecords[(0u + 1u)];
    let _e206 = probeBlendRecords[(0u + 2u)];
    let _e211 = probeBlendRecords[(0u + 3u)];
    let _e216 = probeBlendRecords[(0u + 4u)];
    let _e221 = probeBlendRecords[(0u + 5u)];
    let _e226 = probeBlendRecords[(0u + 6u)];
    let _e231 = probeBlendRecords[(0u + 7u)];
    let _e236 = probeBlendRecords[(0u + 8u)];
    let _e241 = probeBlendRecords[(0u + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e201, _e206, _e211, _e216, _e221, _e226, _e231, _e236, _e241);
    ...
    let _e255 = irradiance;
    let _e257 = diffuseAlbedo;
    let _e258 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e254, (_e255 * skyFactor), probeDiffuseK, _e257, metallic_14);
    if true {
        let _e260 = specularIbl;
        let _e265 = coatF;
        ambient = ((_e258 + (_e260 * _e173)) * (vec3(1f) - _e265));
    }
    let _e268 = ambient;
    ambient = (_e268 * ao);
    let _e270 = reflectionFallback;
    reflectionFallback = ((_e270 * _e173) * ao);
    let _e276 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e279 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e276, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e280 = ambient;
    ambient = (_e280 * _e279);
    let _e282 = reflectionFallback;
    reflectionFallback = (_e282 * _e279);
    let _e284 = ambient;
    color = _e284;
    let _e286 = physicalNormal;
    let _e288 = standardViewZ(in_1);
    let _e289 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e286, in_1.worldPos, _e288);
    let _e290 = physicalNormal;
    let _e291 = diffuseAlbedo;
    let _e292 = f0_;
    let _e293 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e290, v_5, _e291, metallic_14, a_4, _e292);
    let _e294 = color;
    let _e299 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e303 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e307 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e311 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e312 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e289 * _e293), in_1.worldPos, _e299.xyz, _e303.xyz, _e307.xyz, _e311);
    color = (_e294 + _e312);
    let _e314 = color;
    let _e317 = standardViewZ(in_1);
    let _e319 = physicalNormal;
    let _e320 = diffuseAlbedo;
    let _e321 = f0_;
    let _e324 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e317, in_1.worldPos, _e319, v_5, _e320, metallic_14, a_4, _e321, false, true);
    color = (_e314 + _e324);
    let _e326 = color;
    color = (_e326 + _e6.emissive);
    let _e331 = color;
    output.color = vec4<f32>(_e331, alpha_1);
    let _e334 = reflectionFallback;
    let _e335 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e334, _e335);
    let _e337 = output;
    return _e337;
}
