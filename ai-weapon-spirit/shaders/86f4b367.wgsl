...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_11 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_11 = false;
    }
    let _e10 = local_11;
    if _e10 {
        local_12 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_12 = false;
    }
    let _e17 = local_12;
    if _e17 {
        local_13 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_13 = false;
    }
    let validReceiver = local_13;
    ...
    if all((uv_4 >= vec2(0f))) {
        local_14 = all((uv_4 <= vec2(1f)));
    } else {
        local_14 = false;
    }
    let inRange = local_14;
    if (shadowProjection.y > 0.5f) {
        local_15 = (shadowProjection.z < 0.5f);
    } else {
        local_15 = false;
    }
    let _e64 = local_15;
    if _e64 {
        local_16 = (shadowProjection.x > 0f);
    } else {
        local_16 = false;
    }
    let available = local_16;
