...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e220 = dm_rotate((mv.position.xyz * meshSize), angles);
    let forward = vec3<f32>(0f, 0f, 1f);
    let _e232 = dm_rotate((mv.normal.xyz / max(abs(meshSize), vec3(0.00001f))), angles);
    let _e235 = right;
    let _e238 = up;
    let normal = normalize((((_e235 * _e232.x) + (_e238 * _e232.y)) + (forward * _e232.z)));
    let _e246 = center;
    let _e247 = right;
    let _e250 = up;
    let _e260 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e246 + ((((_e247 * _e220.x) + (_e250 * _e220.y)) + (forward * _e220.z)) * _e260));
    let _e267 = streams.uv0_;
    input.uv0_ = _e267;
    let _e270 = streams.uv1_;
    input.custom1_ = _e270;
    let _e273 = p.color;
    input.color = (_e273 * mv.color);
    input.normal = normal;
    let _e278 = world;
    input.worldPosition = _e278;
    let _e282 = frame.eye;
    let _e284 = world;
    input.viewDirection = (_e282.xyz - _e284);
    let _e290 = frame.settings.x;
    input.time = _e290;
    input.delta = 0.008333334f;
    let _e293 = input;
    let _e294 = source_vertex(_e293);
    let _e296 = p.age;
    let _e297 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e294, _e296);
    let _e300 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e297 + _e300.xyz);
    let _e307 = frame.vp;
    let _e308 = world;
    o.position = (_e307 * vec4<f32>(_e308, 1f));
    let _e314 = streams.uv0_;
    o.uv = _e314;
    let _e317 = streams.uv1_;
    o.custom = _e317;
    let _e320 = p.color;
    o.color = (_e320 * mv.color);
    let _e324 = world;
    o.world = _e324;
    let _e328 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e332 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e336 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e328.xyz, _e332.xyz, _e336.xyz) * normal));
    let _e341 = o;
    return _e341;
}
