...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    up = _e99;
    let _e104 = impactDetail.a.y;
    if (_e104 > 0f) {
        let _e108 = size.x;
        let _e112 = impactDetail.a.w;
        let _e116 = impactDetail.a.y;
        size.x = (_e108 * mix(1f, _e112, _e116));
    }
    let _e123 = frame.layer.x;
    let columns = u32(_e123);
    let _e128 = frame.layer.y;
    let rows = u32(_e128);
    let _e132 = p.velocity.w;
    let tile = min(u32(max(_e132, 0f)), ((columns * rows) - 1u));
    let cell_1 = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e154 = frame.extra.x;
    let _e157 = p.id;
    let _e159 = dm_random(bitcast<u32>(_e154), _e157, 210u);
    let _e169 = frame.extra.x;
    let _e172 = p.id;
    let _e174 = dm_random(bitcast<u32>(_e169), _e172, 211u);
    let uvCorner = vec2<f32>(select(_e70.x, -(_e70.x), (_e159 < 0f)), select(_e70.y, -(_e70.y), (_e174 < 0f)));
    let _e185 = frame.layer;
    let _e188 = p;
    let _e192 = frame.extra.x;
    let _e194 = dm_streams((((uvCorner + vec2(0.5f)) + cell_1) / _e185.xy), _e188, bitcast<u32>(_e192));
    let _e197 = frame.layer;
    let _e200 = size;
    let plane = vec3<f32>(((_e70 + _e197.zw) * _e200), 0f);
    let _e207 = p.size_rotation.z;
    let _e212 = dm_rotate(plane, (vec3<f32>(_e194.rotationXY, -(_e207)) * 57.29578f));
    let _e213 = right;
    let _e214 = up;
    let normal = normalize(cross(_e213, _e214));
    let _e217 = center;
    let _e218 = right;
    let _e221 = up;
    let _e231 = frame.settings.w;
    world = (_e217 + ((((_e218 * _e212.x) + (_e221 * _e212.y)) + (normal * _e212.z)) * _e231));
    let _e238 = impactShape.a.x;
    if (_e238 > 0f) {
        let q_1 = (_e70.y + 0.5f);
        let _e245 = p.age;
        let _e247 = p.lifetime;
        let age_1 = clamp((_e245 / _e247), 0f, 1f);
        let _e255 = frame.extra.x;
        let seed_4 = bitcast<u32>(_e255);
        let _e258 = p.id;
        let _e260 = dm_random(seed_4, _e258, 303u);
        let lengthGain = mix(0.72f, 1.05f, _e260);
        let _e267 = impactShape.b.x;
        let width = (_e267 * (0.45f + (0.85f * sin((q_1 * 3.1415927f)))));
        let _e278 = size.y;
        let _e285 = impactShape.a.w;
        let bend = ((((q_1 * q_1) * _e278) * 0.55f) * _e285);
        let _e289 = size.x;
        let _e296 = size.y;
        let _e303 = size.y;
        let _e308 = p.id;
        let blade = vec3<f32>((((_e70.x * _e289) * width) + bend), (((q_1 + 0.06f) * _e296) * lengthGain), (((sin((q_1 * 3.1415927f)) * _e303) * 0.36f) + (((f32((_e308 % 3u)) - 1f) * q_1) * 0.22f)));
        let _e321 = p.size_rotation.z;
        let angle_2 = (-(_e321) + ((age_1 - 0.25f) * 0.35f));
        let turned = vec3<f32>(((cos(angle_2) * blade.x) - (sin(angle_2) * blade.y)), ((sin(angle_2) * blade.x) + (cos(angle_2) * blade.y)), blade.z);
        let _e345 = p.id;
        let _e349 = impactShape.a.z;
        let _e350 = shapePlane(turned, _e345, _e349);
        let _e351 = right;
        let _e354 = up;
        let facing = ((_e351 * turned.x) + (_e354 * turned.y));
        let _e358 = world;
        let _e359 = center;
        let _e363 = impactShape.a.z;
        let _e370 = frame.settings.w;
        let _e376 = impactShape.a.x;
        world = mix(_e358, (_e359 + (mix(facing, _e350, min(1f, _e363)) * _e370)), _e376);
    }
    input.uv0_ = _e194.uv0_;
    input.custom1_ = _e194.uv1_;
    let _e385 = p.color;
    input.color = _e385;
    input.normal = normal;
    let _e388 = world;
    input.worldPosition = _e388;
    let _e393 = frame.settings.x;
    input.time = _e393;
    input.delta = 0.008333334f;
    let _e396 = input;
    let _e397 = source_vertex(_e396);
    let _e399 = p.age;
    let _e400 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e397, _e399);
    let _e403 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e400 + _e403.xyz);
    let _e410 = frame.vp;
    let _e411 = world;
    o.position = (_e410 * vec4<f32>(_e411, 1f));
    o.uv = _e194.uv0_;
    o.custom = _e194.uv1_;
    let _e421 = p.color;
    o.color = _e421;
    let _e423 = world;
    let _e426 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    o.world = (_e423 - _e426.xyz);
    o.normal = normal;
    let _e430 = o;
    return _e430;
}
