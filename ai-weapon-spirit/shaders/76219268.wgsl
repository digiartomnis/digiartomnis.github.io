...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e155 = f0_;
    let _e156 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e151, v), 0f), _e155, iblRoughness);
    ...
    if (_e168 < 0f) {
        ...
        let _e227 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e179, v, iblRoughness, _e180, in_2.worldPos, vec3<f32>(_e184, _e187, _e190), _e194.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1, skylightPrefilterMap, _e203, _e206.xyz, max((-(_e210) - 1f), 0f), (_e219 > 0.5f));
        specularIbl = _e227;
    } else {
        ...
        let _e245 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e236, v, iblRoughness, _e237, _e240, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e245;
    }
    ...
    let _e374 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e370, (_e371 * skyFactor), probeDiffuseK, _e373, metallic_4);
    ...
    let _e458 = material.thickness;
    let _e460 = finiteScalar(_e458, 0f);
    let _e463 = thicknessSample;
    let _e465 = finiteScalar(_e463, 1f);
    let worldThickness = max(((_e460 * length(worldRefractedDirection)) * _e465), 0f);
    ...
    let _e507 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e507.xyz;
    ...
    let _e515 = local_2;
    if _e515 {
        let _e523 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e523.xyz;
    }
    let _e527 = material.attenuationColor;
    let _e530 = finiteColor(_e527, vec3(1f));
    let safeAttenuationColor = clamp(_e530, vec3(0f), vec3(1f));
    let _e538 = material.attenuationDistance;
    let _e540 = finiteScalar(_e538, 0f);
    let safeAttenuationDistance = max(_e540, 0f);
    ...
    let _e557 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e557);
    let _e559 = color;
    let _e560 = transmittedBackdrop;
    let _e563 = finiteColor(_e560, vec3(0f));
    color = (_e559 + ((_e563 * transmittedEnergy) * beerAttenuation));
    let _e567 = color;
    color = (_e567 + _e7.emissive);
    ...
    let _e576 = f0_;
    let _e577 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e574, v, _e575, metallic_4, a_2, _e576);
    let _e578 = color;
