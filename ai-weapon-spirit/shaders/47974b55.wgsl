...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e295 = probeBlendRecords[(0u + 1u)];
    let _e300 = probeBlendRecords[(0u + 2u)];
    let _e305 = probeBlendRecords[(0u + 3u)];
    let _e310 = probeBlendRecords[(0u + 4u)];
    let _e315 = probeBlendRecords[(0u + 5u)];
    let _e320 = probeBlendRecords[(0u + 6u)];
    let _e325 = probeBlendRecords[(0u + 7u)];
    let _e330 = probeBlendRecords[(0u + 8u)];
    let _e335 = probeBlendRecords[(0u + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e295, _e300, _e305, _e310, _e315, _e320, _e325, _e330, _e335);
    let probeLocalBlendFraction = probeBlendRecords[0u].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_12)), vec3(0.0001f)));
    let _e348 = physicalNormal;
    let _e349 = irradiance;
    let _e351 = diffuseAlbedo;
    let _e352 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e348, (_e349 * skyFactor), probeDiffuseK, _e351, metallic_12);
    if true {
        let _e354 = specularIbl;
        let _e359 = coatF;
        ambient = ((_e352 + (_e354 * _e274)) * (vec3(1f) - _e359));
    }
    let _e362 = ambient;
    ambient = (_e362 * ao);
    let _e364 = ambient;
    let _e368 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e371 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e368, ssaoBlurredTexture, ssaoBlurredSampler);
    ambient = (_e364 * _e371);
    let _e373 = ambient;
    color = _e373;
    ...
    let _e446 = material.thickness;
    let _e448 = finiteScalar(_e446, 0f);
    let _e451 = thicknessSample;
    let _e453 = finiteScalar(_e451, 1f);
    let worldThickness = max(((_e448 * length(worldRefractedDirection)) * _e453), 0f);
    ...
    let _e495 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e495.xyz;
    ...
    let _e503 = local_1;
    if _e503 {
        let _e511 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e511.xyz;
    }
    let _e515 = material.attenuationColor;
    let _e518 = finiteColor(_e515, vec3(1f));
    let safeAttenuationColor = clamp(_e518, vec3(0f), vec3(1f));
    let _e526 = material.attenuationDistance;
    let _e528 = finiteScalar(_e526, 0f);
    let safeAttenuationDistance = max(_e528, 0f);
    ...
    let _e545 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e545);
    let _e547 = color;
    let _e548 = transmittedBackdrop;
    let _e551 = finiteColor(_e548, vec3(0f));
    color = (_e547 + ((_e551 * transmittedEnergy) * beerAttenuation));
    let _e555 = color;
    color = (_e555 + _e7.emissive);
    let _e558 = physicalNormal;
    let _e560 = standardViewZ(in_1);
    let _e561 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e558, in_1.worldPos, _e560);
    let _e562 = physicalNormal;
    let _e563 = diffuseAlbedo;
    let _e564 = f0_;
    let _e565 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e562, v_2, _e563, metallic_12, a_2, _e564);
    let _e566 = color;
    let _e571 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e575 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e579 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e583 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e584 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e561 * _e565), in_1.worldPos, _e571.xyz, _e575.xyz, _e579.xyz, _e583);
    color = (_e566 + _e584);
    let _e586 = color;
    let _e589 = standardViewZ(in_1);
    let _e591 = physicalNormal;
    let _e592 = diffuseAlbedo;
    let _e593 = f0_;
    let _e596 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e589, in_1.worldPos, _e591, v_2, _e592, metallic_12, a_2, _e593, false, true);
    color = (_e586 + _e596);
    let _e598 = color;
    return vec4<f32>(_e598, alpha);
}
