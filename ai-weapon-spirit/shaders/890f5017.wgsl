...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_18 = normalize(input_2.geometricNormalWS);
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e286 = coatF;
    ambient = (((((kD_1 * _e275) * _e277) * skyFactor) + (_e280 * _e274)) * (vec3(1f) - _e286));
    ...
    let _e317 = probeBlendRecords[(probeBase + 1u)];
    let _e322 = probeBlendRecords[(probeBase + 2u)];
    let _e327 = probeBlendRecords[(probeBase + 3u)];
    let _e332 = probeBlendRecords[(probeBase + 4u)];
    let _e337 = probeBlendRecords[(probeBase + 5u)];
    let _e342 = probeBlendRecords[(probeBase + 6u)];
    let _e347 = probeBlendRecords[(probeBase + 7u)];
    let _e352 = probeBlendRecords[(probeBase + 8u)];
    let _e357 = probeBlendRecords[(probeBase + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e317, _e322, _e327, _e332, _e337, _e342, _e347, _e352, _e357);
    let probeLocalBlendFraction = probeBlendRecords[probeBase].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_13)), vec3(0.0001f)));
    let _e370 = physicalNormal;
    let _e371 = irradiance;
    let _e373 = diffuseAlbedo;
    let _e374 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e370, (_e371 * skyFactor), probeDiffuseK, _e373, metallic_13);
    if probeIdentityValid {
        let _e375 = specularIbl;
        let _e380 = coatF;
        ambient = ((_e374 + (_e375 * _e274)) * (vec3(1f) - _e380));
    }
    let _e383 = ambient;
    ambient = (_e383 * ao);
    let _e385 = ambient;
    let _e389 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e392 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e389, ssaoBlurredTexture, ssaoBlurredSampler);
    ambient = (_e385 * _e392);
    let _e394 = ambient;
    color = _e394;
    ...
    let _e467 = material.thickness;
    let _e469 = finiteScalar(_e467, 0f);
    let _e472 = thicknessSample;
    let _e474 = finiteScalar(_e472, 1f);
    let worldThickness = max(((_e469 * length(worldRefractedDirection)) * _e474), 0f);
    ...
    let _e516 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e516.xyz;
    ...
    let _e524 = local_2;
    if _e524 {
        let _e532 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e532.xyz;
    }
    let _e536 = material.attenuationColor;
    let _e539 = finiteColor(_e536, vec3(1f));
    let safeAttenuationColor = clamp(_e539, vec3(0f), vec3(1f));
    let _e547 = material.attenuationDistance;
    let _e549 = finiteScalar(_e547, 0f);
    let safeAttenuationDistance = max(_e549, 0f);
    ...
    let _e566 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e566);
    let _e568 = color;
    let _e569 = transmittedBackdrop;
    let _e572 = finiteColor(_e569, vec3(0f));
    color = (_e568 + ((_e572 * transmittedEnergy) * beerAttenuation));
    let _e576 = color;
    color = (_e576 + _e7.emissive);
    let _e579 = physicalNormal;
    let _e581 = standardViewZ(in_2);
    let _e582 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e579, in_2.worldPos, _e581);
    let _e583 = physicalNormal;
    let _e584 = diffuseAlbedo;
    let _e585 = f0_;
    let _e586 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e583, v_5, _e584, metallic_13, a_3, _e585);
    let _e587 = color;
    let _e592 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e596 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e600 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e604 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e605 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e582 * _e586), in_2.worldPos, _e592.xyz, _e596.xyz, _e600.xyz, _e604);
    color = (_e587 + _e605);
    let _e607 = color;
    let _e610 = standardViewZ(in_2);
    let _e612 = physicalNormal;
    let _e613 = diffuseAlbedo;
    let _e614 = f0_;
    let _e617 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e610, in_2.worldPos, _e612, v_5, _e613, metallic_13, a_3, _e614, false, true);
    color = (_e607 + _e617);
    let _e619 = color;
    return vec4<f32>(_e619, alpha);
}
