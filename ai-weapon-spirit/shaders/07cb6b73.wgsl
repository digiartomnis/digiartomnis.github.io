...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_33 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_33 = false;
    }
    let _e10 = local_33;
    if _e10 {
        local_34 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_34 = false;
    }
    let _e17 = local_34;
    if _e17 {
        local_35 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_35 = false;
    }
    let validReceiver = local_35;
    ...
    if all((uv_7 >= vec2(0f))) {
        local_36 = all((uv_7 <= vec2(1f)));
    } else {
        local_36 = false;
    }
    let inRange = local_36;
    if (shadowProjection.y > 0.5f) {
        local_37 = (shadowProjection.z < 0.5f);
    } else {
        local_37 = false;
    }
    let _e64 = local_37;
    if _e64 {
        local_38 = (shadowProjection.x > 0f);
    } else {
        local_38 = false;
    }
    let available = local_38;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e363 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e359, (_e360 * skyFactor), probeDiffuseK, _e362, metallic_13);
    ...
    let _e576 = f0_;
    let _e577 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e574, v_2, _e575, metallic_13, a_3, _e576);
    let _e578 = color;
    ...
    let _e605 = f0_;
    let _e608 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e601, in_2.worldPos, _e603, v_2, _e604, metallic_13, a_3, _e605, false, true);
    color = (_e598 + _e608);
    let _e610 = color;
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_3, normalize((_e28 - in_3.worldPos)), roughness_9, _e32, brdfLut_3, irradianceSampler_1);
