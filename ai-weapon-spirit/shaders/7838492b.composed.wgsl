...
fn gradient_6_(t_4: f32) -> vec4<f32> {
    ...
    if (t_4 <= 0f) {
        ...
        if (t_4 <= 0.29117265f) {
            rgb_2 = mix(vec3<f32>(0.18867922f, 0.00978996f, 0.00978996f), vec3<f32>(0.3773585f, 0f, 0.011199376f), ((t_4 - 0f) / 0.29117265f));
        } else {
            if (t_4 <= 0.52059203f) {
                rgb_2 = mix(vec3<f32>(0.3773585f, 0f, 0.011199376f), vec3<f32>(0.7924528f, 0.08505074f, 0.03364185f), ((t_4 - 0.29117265f) / 0.2294194f));
            } else {
                if (t_4 <= 0.9529412f) {
                    rgb_2 = mix(vec3<f32>(0.7924528f, 0.08505074f, 0.03364185f), vec3<f32>(0.4433962f, 0.008424807f, 0f), ((t_4 - 0.52059203f) / 0.43234912f));
                }
            }
        }
    }
...
fn gradient_7_(t_5: f32) -> vec4<f32> {
    ...
    if (t_5 <= 0f) {
        ...
        if (t_5 <= 1f) {
            rgb_3 = mix(vec3<f32>(0.9056604f, 0.2264151f, 0.34850165f), vec3<f32>(0.5019608f, 0f, 0f), ((t_5 - 0f) / 1f));
        }
    }
    if (t_5 <= 0f) {
        alpha_2 = 1f;
        ...
        if (t_5 <= 0.74412143f) {
            alpha_2 = mix(1f, 0.5450981f, ((t_5 - 0f) / 0.74412143f));
        } else {
            if (t_5 <= 1f) {
                alpha_2 = mix(0.5450981f, 0f, ((t_5 - 0.74412143f) / 0.25587854f));
            }
        }
    }
    let _e48 = rgb_3;
...
fn gradient_8_(t_6: f32) -> vec4<f32> {
    ...
    if (t_6 <= 0f) {
        ...
        if (t_6 <= 1f) {
            rgb_4 = mix(vec3<f32>(0.8018868f, 0.011347434f, 0.011347434f), vec3<f32>(0.3019608f, 0f, 0f), ((t_6 - 0f) / 1f));
        }
    }
    if (t_6 <= 0f) {
        alpha_3 = 1f;
        ...
        if (t_6 <= 0.6205844f) {
            alpha_3 = mix(1f, 0.2235294f, ((t_6 - 0f) / 0.6205844f));
        } else {
            if (t_6 <= 1f) {
                alpha_3 = mix(0.2235294f, 0f, ((t_6 - 0.6205844f) / 0.37941557f));
            }
        }
    }
    let _e48 = rgb_4;
...
fn dm_streams(uv: vec2<f32>, p_1: Particle, seed_2: u32) -> DmStreams {
    ...
    let _e14 = curve_0_(t_11);
    let _e15 = curve_1_(t_11);
    let _e17 = dm_random(seed_2, id_2, 161u);
    let _e22 = dm_random(seed_2, id_2, 162u);
...
fn curve_5_(t_10: f32) -> f32 {
    if (t_10 <= 0f) {
        return 0.2837839f;
    }
    if (t_10 < 0.214352f) {
        let u_6 = ((t_10 - 0f) / 0.214352f);
...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e74 = p.position;
    let _e78 = frame.motion;
    let _e83 = frame.settings.x;
    center = (_e74.xyz + (_e78.xyz * _e83));
    let _e88 = p.size_rotation;
    size = _e88.xy;
    let _e93 = frame.right;
    let _e95 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e93.xyz);
    right = _e95;
    let _e99 = frame.up;
    let _e101 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e99.xyz);
    up = _e101;
    let _e106 = frame.layer.x;
    let columns = u32(_e106);
    let _e111 = frame.layer.y;
    let rows = u32(_e111);
    let _e115 = p.velocity.w;
    let tile = min(u32(max(_e115, 0f)), ((columns * rows) - 1u));
    let cell = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e137 = frame.extra.x;
    let _e140 = p.id;
    let _e142 = dm_random(bitcast<u32>(_e137), _e140, 210u);
    let _e152 = frame.extra.x;
    let _e155 = p.id;
    let _e157 = dm_random(bitcast<u32>(_e152), _e155, 211u);
    let uvCorner = vec2<f32>(select(c_1.x, -(c_1.x), (_e142 < 0f)), select(c_1.y, -(c_1.y), (_e157 < 0f)));
    let _e168 = frame.layer;
    let _e171 = p;
    let _e175 = frame.extra.x;
    let _e177 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e168.xy), _e171, bitcast<u32>(_e175));
    let _e180 = frame.layer;
    let _e183 = size;
    let plane = vec3<f32>(((c_1 + _e180.zw) * _e183), 0f);
    let _e190 = p.size_rotation.z;
    let _e195 = dm_rotate(plane, (vec3<f32>(_e177.rotationXY, -(_e190)) * 57.29578f));
    let _e196 = right;
    let _e197 = up;
    let normal = normalize(cross(_e196, _e197));
    let _e200 = center;
    let _e201 = right;
    let _e204 = up;
    let _e214 = frame.settings.w;
    world = (_e200 + ((((_e201 * _e195.x) + (_e204 * _e195.y)) + (normal * _e195.z)) * _e214));
    input.uv0_ = _e177.uv0_;
    input.custom1_ = _e177.uv1_;
    let _e225 = p.color;
    input.color = _e225;
    input.normal = normal;
    let _e228 = world;
    input.worldPosition = _e228;
    let _e233 = frame.settings.x;
    input.time = _e233;
    input.delta = 0.008333334f;
    let _e236 = input;
    let _e237 = source_vertex(_e236);
    let _e239 = p.age;
    let _e240 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e237, _e239);
    let _e243 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e240 + _e243.xyz);
    let _e250 = frame.vp;
    let _e251 = world;
    o.position = (_e250 * vec4<f32>(_e251, 1f));
    o.uv = _e177.uv0_;
    o.custom = _e177.uv1_;
    let _e261 = p.color;
    o.color = _e261;
    let _e263 = world;
    let _e266 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    o.world = (_e263 - _e266.xyz);
    o.normal = normal;
    let _e270 = o;
    return _e270;
}
