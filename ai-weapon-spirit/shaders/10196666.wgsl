...
fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_7: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_8: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_10 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_10 = true;
        }
        let _e90 = local_10;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_11 = (cookieUv.x > 1f);
            } else {
                local_11 = true;
            }
            let _e137 = local_11;
            if !(_e137) {
                local_12 = (cookieUv.y < 0f);
            } else {
                local_12 = true;
            }
            let _e145 = local_12;
            if !(_e145) {
                local_13 = (cookieUv.y > 1f);
            } else {
                local_13 = true;
            }
            let _e153 = local_13;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_5: u32, normal_15: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_6 >= (tileLo + texel_5))) {
            local_36 = all((uv_6 <= (tileHi - texel_5)));
        } else {
            local_36 = false;
        }
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_38 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_38 = false;
    }
    let _e10 = local_38;
    if _e10 {
        local_39 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_39 = false;
    }
    let _e17 = local_39;
    if _e17 {
        local_40 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_40 = false;
    }
    let validReceiver = local_40;
    ...
    if all((uv_7 >= vec2(0f))) {
        local_41 = all((uv_7 <= vec2(1f)));
    } else {
        local_41 = false;
    }
    let inRange = local_41;
    if (shadowProjection.y > 0.5f) {
        local_42 = (shadowProjection.z < 0.5f);
    } else {
        local_42 = false;
    }
    let _e64 = local_42;
    if _e64 {
        local_43 = (shadowProjection.x > 0f);
    } else {
        local_43 = false;
    }
    let available = local_43;
...
fn composeProbeDiffuse(shPreblend_2: array<vec4<f32>, 9>, localBlendFraction_1: f32, normal_18: vec3<f32>, skyIrradiance: vec3<f32>, kD: vec3<f32>, albedo_1: vec3<f32>, metallic_11: f32) -> vec3<f32> {
    let _e7 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_2, localBlendFraction_1, normal_18, skyIrradiance, kD, albedo_1, metallic_11);
    return _e7;
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e137 = f0_;
    let _e138 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(_e133, v_5), 0f), _e137, iblRoughness);
    ...
    if (_e150 < 0f) {
        ...
        let _e156 = skylight.diffuseRotation;
        let _e159 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e153, _e156, irradianceMap_1, irradianceSampler_1);
        irradiance = _e159;
        let _e161 = physicalNormal;
        let _e162 = f0_;
        let _e166 = skylight.colorR;
        let _e169 = skylight.colorG;
        let _e172 = skylight.colorB;
        let _e176 = skylight.rotation;
        let _e185 = skylight.diffuseRotation;
        let _e188 = skylight.diffuseScale;
        let _e192 = skylight.intensity;
        let _e201 = skylight.rotation.w;
        let _e209 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e161, v_5, iblRoughness, _e162, in_2.worldPos, vec3<f32>(_e166, _e169, _e172), _e176.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1, skylightPrefilterMap, _e185, _e188.xyz, max((-(_e192) - 1f), 0f), (_e201 > 0.5f));
        specularIbl = _e209;
    } else {
        let _e211 = physicalNormal;
        let _e214 = skylight.rotation;
        let _e217 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e211, _e214, irradianceMap_1, irradianceSampler_1);
        irradiance = _e217;
        let _e218 = physicalNormal;
        let _e219 = f0_;
        let _e222 = skylight.rotation;
        let _e227 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e218, v_5, iblRoughness, _e219, _e222, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specularIbl = _e227;
    }
    let ao = _e7.occlusion;
    let _e231 = skylight.colorR;
    let _e234 = skylight.colorG;
    let _e237 = skylight.colorB;
    let skyColor_1 = vec3<f32>(_e231, _e234, _e237);
    let _e241 = skylight.intensity;
    let _e245 = skylight.diffuseScale;
    let _e249 = skylight.intensity;
    let skyFactor = select((skyColor_1 * _e241), _e245.xyz, (_e249 < 0f));
    let _e255 = skylight.intensity;
    let _e256 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(skyColor_1, _e255);
    let _e257 = specularIbl;
    let _e261 = coatF;
    reflectionFallback = (_e257 * (vec3(1f) - _e261));
    let _e265 = irradiance;
    let _e267 = diffuseAlbedo;
    let _e270 = specularIbl;
    let _e275 = coatF;
    ambient = (((((kD_1 * _e265) * _e267) * skyFactor) + (_e270 * _e256)) * (vec3(1f) - _e275));
    ...
    let _e306 = probeBlendRecords[(probeBase + 1u)];
    let _e311 = probeBlendRecords[(probeBase + 2u)];
    let _e316 = probeBlendRecords[(probeBase + 3u)];
    let _e321 = probeBlendRecords[(probeBase + 4u)];
    let _e326 = probeBlendRecords[(probeBase + 5u)];
    let _e331 = probeBlendRecords[(probeBase + 6u)];
    let _e336 = probeBlendRecords[(probeBase + 7u)];
    let _e341 = probeBlendRecords[(probeBase + 8u)];
    let _e346 = probeBlendRecords[(probeBase + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e306, _e311, _e316, _e321, _e326, _e331, _e336, _e341, _e346);
    let probeLocalBlendFraction = probeBlendRecords[probeBase].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_14)), vec3(0.0001f)));
    let _e359 = physicalNormal;
    let _e360 = irradiance;
    let _e362 = diffuseAlbedo;
    let _e363 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e359, (_e360 * skyFactor), probeDiffuseK, _e362, metallic_14);
    if probeIdentityValid {
        let _e364 = specularIbl;
        let _e369 = coatF;
        ambient = ((_e363 + (_e364 * _e256)) * (vec3(1f) - _e369));
    }
    let _e372 = ambient;
    ambient = (_e372 * ao);
    let _e374 = reflectionFallback;
    reflectionFallback = ((_e374 * _e256) * ao);
    let _e380 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e383 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e380, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e384 = ambient;
    ambient = (_e384 * _e383);
    let _e386 = reflectionFallback;
    reflectionFallback = (_e386 * _e383);
    let _e388 = ambient;
    color = _e388;
    ...
    let _e461 = material.thickness;
    let _e463 = finiteScalar(_e461, 0f);
    let _e466 = thicknessSample;
    let _e468 = finiteScalar(_e466, 1f);
    let worldThickness = max(((_e463 * length(worldRefractedDirection)) * _e468), 0f);
    ...
    let _e510 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e510.xyz;
    ...
    let _e518 = local_2;
    if _e518 {
        let _e526 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e526.xyz;
    }
    let _e530 = material.attenuationColor;
    let _e533 = finiteColor(_e530, vec3(1f));
    let safeAttenuationColor = clamp(_e533, vec3(0f), vec3(1f));
    let _e541 = material.attenuationDistance;
    let _e543 = finiteScalar(_e541, 0f);
    let safeAttenuationDistance = max(_e543, 0f);
    ...
    let _e560 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e560);
    let _e562 = color;
    let _e563 = transmittedBackdrop;
    let _e566 = finiteColor(_e563, vec3(0f));
    color = (_e562 + ((_e566 * transmittedEnergy) * beerAttenuation));
    let _e570 = physicalNormal;
    let _e572 = standardViewZ(in_2);
    let _e573 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e570, in_2.worldPos, _e572);
    let _e574 = physicalNormal;
    let _e575 = diffuseAlbedo;
    let _e576 = f0_;
    let _e577 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e574, v_5, _e575, metallic_14, a_4, _e576);
    let _e578 = color;
    let _e583 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e587 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e591 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e595 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e596 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e573 * _e577), in_2.worldPos, _e583.xyz, _e587.xyz, _e591.xyz, _e595);
    color = (_e578 + _e596);
    let _e598 = color;
    let _e601 = standardViewZ(in_2);
    let _e603 = physicalNormal;
    let _e604 = diffuseAlbedo;
    let _e605 = f0_;
    let _e608 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e601, in_2.worldPos, _e603, v_5, _e604, metallic_14, a_4, _e605, false, true);
    color = (_e598 + _e608);
    let _e610 = color;
    color = (_e610 + _e7.emissive);
    let _e615 = color;
    output.color = vec4<f32>(_e615, alpha_1);
    let _e618 = reflectionFallback;
    let _e619 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e618, _e619);
    let _e621 = output;
    return _e621;
}
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_6, normalize((_e28 - in_3.worldPos)), roughness_10, _e32, brdfLut_3, irradianceSampler_1);
