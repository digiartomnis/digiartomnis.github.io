...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e49 = frame.invvp;
    let origin = (_e49 * vec4<f32>(native.center, 1f));
    let _e61 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    let _e64 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(((origin.xyz / vec3(origin.w)) - _e61.xyz));
    p.position = vec4<f32>(_e64, 1f);
    ...
    let _e103 = p.position;
    let _e107 = frame.motion;
    let _e112 = frame.settings.x;
    center = (_e103.xyz + (_e107.xyz * _e112));
    let _e117 = p.size_rotation;
    size = _e117.xy;
    ...
    let uvCorner = vec2<f32>(select(c_1.x, -(c_1.x), (_e159 < 0f)), select(c_1.y, -(c_1.y), (_e174 < 0f)));
    let _e185 = frame.layer;
    let _e188 = p;
    let _e192 = frame.extra.x;
    let _e194 = dm_streams((((uvCorner + vec2(0.5f)) + cell) / _e185.xy), _e188, bitcast<u32>(_e192));
    streams = _e194;
    let _e198 = streams.uv0_;
    streams.uv0_ = vec4<f32>(_e198.xy, mv.uv.zw);
    let _e203 = size;
    let _e206 = p.size_rotation.w;
    let meshSize = vec3<f32>(_e203, _e206);
    let _e209 = streams.rotationXY;
    let _e212 = p.size_rotation.z;
    let angles = (vec3<f32>(_e209, -(_e212)) * 57.29578f);
    let _e220 = p;
    let _e224 = frame.extra.x;
    let _e226 = ue_mesh_rotate((mv.position.xyz * meshSize), _e220, bitcast<u32>(_e224));
    let forward = vec3<f32>(0f, 0f, 1f);
    let _e238 = p;
    let _e242 = frame.extra.x;
    let _e244 = ue_mesh_rotate((mv.normal.xyz / max(abs(meshSize), vec3(0.00001f))), _e238, bitcast<u32>(_e242));
    let _e247 = right;
    let _e250 = up;
    let normal = normalize((((_e247 * _e244.x) + (_e250 * _e244.y)) + (forward * _e244.z)));
    let _e258 = center;
    let _e259 = right;
    let _e262 = up;
    let _e272 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.settings.w;
    world = (_e258 + ((((_e259 * _e226.x) + (_e262 * _e226.y)) + (forward * _e226.z)) * _e272));
    let _e279 = streams.uv0_;
    input.uv0_ = _e279;
    let _e282 = streams.uv1_;
    input.custom1_ = _e282;
    let _e285 = p.color;
    input.color = (_e285 * mv.color);
    input.normal = normal;
    let _e290 = world;
    input.worldPosition = _e290;
    let _e294 = frame.eye;
    let _e296 = world;
    input.viewDirection = (_e294.xyz - _e296);
    let _e302 = frame.settings.x;
    input.time = _e302;
    input.delta = 0.008333334f;
    let _e305 = input;
    let _e306 = source_vertex(_e305);
    let _e308 = p.age;
    let _e309 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e306, _e308);
    let _e312 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e309 + _e312.xyz);
    let _e319 = frame.vp;
    let _e320 = world;
    o.position = (_e319 * vec4<f32>(_e320, 1f));
    let _e326 = streams.uv0_;
    o.uv = _e326;
    let _e329 = streams.uv1_;
    o.custom = _e329;
    let _e332 = p.color;
    o.color = (_e332 * mv.color);
    let _e336 = world;
    o.world = _e336;
    let _e340 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisX;
    let _e344 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisY;
    let _e348 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.basisZ;
    o.normal = normalize((mat3x3<f32>(_e340.xyz, _e344.xyz, _e348.xyz) * normal));
    let _e353 = o;
    return _e353;
}
