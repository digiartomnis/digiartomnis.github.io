...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_4: u32, count_4: u32, normal_12: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_14: bool;
    var local_15: bool;
    var local_16: bool;
    var local_17: bool;
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_13: vec3<f32>, worldPos_12: vec3<f32>, viewZ: f32) -> f32 {
    ...
    let receiverPosition = (worldPos_12 + (normal_13 * _e33));
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_20 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_20 = false;
    }
    let _e10 = local_20;
    if _e10 {
        local_21 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_21 = false;
    }
    let _e17 = local_21;
    if _e17 {
        local_22 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_22 = false;
    }
    let validReceiver = local_22;
    ...
    if all((uv_6 >= vec2(0f))) {
        local_23 = all((uv_6 <= vec2(1f)));
    } else {
        local_23 = false;
    }
    let inRange = local_23;
    if (shadowProjection.y > 0.5f) {
        local_24 = (shadowProjection.z < 0.5f);
    } else {
        local_24 = false;
    }
    let _e64 = local_24;
    if _e64 {
        local_25 = (shadowProjection.x > 0f);
    } else {
        local_25 = false;
    }
    let available = local_25;
