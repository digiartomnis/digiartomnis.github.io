...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e33 = frame.invvp;
    let origin = (_e33 * vec4<f32>(native.center, 1f));
    let _e45 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    p.position = vec4<f32>(((origin.xyz / vec3(origin.w)) - _e45.xyz), 1f);
    ...
    let _e70 = shapeCorner(vertex);
    let _e72 = p.position;
    let _e76 = frame.motion;
    let _e81 = frame.settings.x;
    center = (_e72.xyz + (_e76.xyz * _e81));
    let _e86 = p.size_rotation;
    size = _e86.xy;
    let _e91 = frame.right;
    let _e93 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e91.xyz);
    right = _e93;
    let _e97 = frame.up;
    let _e99 = magic_local_axisX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e97.xyz);
    up = _e99;
    let _e104 = frame.layer.x;
    let columns = u32(_e104);
    let _e109 = frame.layer.y;
    let rows = u32(_e109);
    let _e113 = p.velocity.w;
    let tile = min(u32(max(_e113, 0f)), ((columns * rows) - 1u));
    let cell_1 = vec2<f32>(f32((tile % columns)), f32(((rows - 1u) - (tile / columns))));
    let _e135 = frame.extra.x;
    let _e138 = p.id;
    let _e140 = dm_random(bitcast<u32>(_e135), _e138, 210u);
    let _e150 = frame.extra.x;
    let _e153 = p.id;
    let _e155 = dm_random(bitcast<u32>(_e150), _e153, 211u);
    let uvCorner = vec2<f32>(select(_e70.x, -(_e70.x), (_e140 < 0f)), select(_e70.y, -(_e70.y), (_e155 < 0f)));
    let _e166 = frame.layer;
    let _e169 = p;
    let _e173 = frame.extra.x;
    let _e175 = dm_streams((((uvCorner + vec2(0.5f)) + cell_1) / _e166.xy), _e169, bitcast<u32>(_e173));
    let _e178 = frame.layer;
    let _e181 = size;
    let plane = vec3<f32>(((_e70 + _e178.zw) * _e181), 0f);
    let _e188 = p.size_rotation.z;
    let _e193 = dm_rotate(plane, (vec3<f32>(_e175.rotationXY, -(_e188)) * 57.29578f));
    let _e194 = right;
    let _e195 = up;
    let normal = normalize(cross(_e194, _e195));
    let _e198 = center;
    let _e199 = right;
    let _e202 = up;
    let _e212 = frame.settings.w;
    world = (_e198 + ((((_e199 * _e193.x) + (_e202 * _e193.y)) + (normal * _e193.z)) * _e212));
    let _e219 = impactShape.a.x;
    if (_e219 > 0f) {
        let _e223 = p.age;
        let _e225 = p.lifetime;
        let age_1 = clamp((_e223 / _e225), 0f, 1f);
        let _e233 = frame.extra.x;
        let seed_4 = bitcast<u32>(_e233);
        let _e237 = p.size_rotation.z;
        let phase = (-(_e237) + ((age_1 - 0.2f) * 0.95f));
        let _e252 = impactShape.a.y;
        let _e259 = p.id;
        let sweep = (((((_e70.x + 0.5f) * 2.5f) * _e252) + phase) + (0.12f * sin(((_e70.y * 5f) + f32(_e259)))));
        let _e267 = size.x;
        let radius_1 = (max(_e267, 0.03f) * (0.43f + (_e70.y * 0.58f)));
        let _e282 = p.id;
        let ripple = (1f + (0.09f * sin(((((_e70.x + 0.5f) * 15f) + (f32(_e282) * 2.3f)) + (age_1 * 8f)))));
        let _e308 = p.id;
        let _e313 = size.x;
        let ribbon = vec3<f32>((((cos(sweep) * radius_1) * ripple) * 1.12f), (((sin(sweep) * radius_1) * ripple) * 0.82f), ((sin(((sweep * 1.4f) + f32(_e308))) * _e313) * 0.24f));
        let knot = min(u32(((_e70.x + 0.5f) * 6f)), 5u);
        let along = clamp((((_e70.x + 0.5f) * 6f) - f32(knot)), 0f, 1f);
        let _e337 = p.id;
        let _e338 = boltPoint(knot, _e337, seed_4);
        let _e342 = p.id;
        let _e343 = boltPoint((knot + 1u), _e342, seed_4);
        let direction = normalize((_e343 - _e338));
        na = vec2<f32>(-(direction.y), direction.x);
        let _e351 = na;
        nb = _e351;
        if (knot > 0u) {
            let _e358 = p.id;
            let _e359 = boltPoint((knot - 1u), _e358, seed_4);
            let prev = normalize((_e338 - _e359));
            let n_1 = vec2<f32>(-(prev.y), prev.x);
            let _e366 = na;
            let join = normalize((n_1 + _e366));
            let _e369 = na;
            na = (join / vec2(max(0.55f, dot(join, _e369))));
        }
        if (knot < 5u) {
            let _e380 = p.id;
            let _e381 = boltPoint((knot + 2u), _e380, seed_4);
            let next = normalize((_e381 - _e343));
        ...
        let _e400 = p.id;
        let lane = (f32((_e400 % 3u)) - 1f);
        let _e410 = impactShape.b.z;
        let _e416 = na;
        let _e417 = nb;
        let line = ((mix(_e338, _e343, along) * (1f + (_e410 * 0.2f))) + ((mix(_e416, _e417, along) * _e70.y) * 0.11f));
        let rotated = vec2<f32>(((cos(phase) * line.x) - (sin(phase) * line.y)), ((sin(phase) * line.x) + (cos(phase) * line.y)));
        let _e440 = size.x;
        let boltSize = min((_e440 * 0.65f), 2.2f);
        let _e456 = impactShape.b.z;
        let bolt = vec3<f32>((rotated * boltSize), (((mix(_e338.y, _e343.y, along) * 0.25f) + ((lane * 0.35f) * _e456)) * boltSize));
        let _e464 = impactShape.b.y;
        let surface = mix(ribbon, bolt, _e464);
        let _e467 = p.id;
        let _e471 = impactShape.a.z;
        let _e472 = shapePlane(surface, _e467, _e471);
        let _e473 = right;
        let _e476 = up;
        let facing = ((_e473 * surface.x) + (_e476 * surface.y));
        let _e483 = impactShape.a.z;
        let spatial = mix(facing, _e472, min(1f, _e483));
        let _e487 = center;
        let _e493 = impactShape.b.z;
        let _e499 = impactShape.b.y;
        let _e507 = impactShape.a.z;
        let anchor = ((_e487 * 0.65f) + vec3<f32>(0f, 0.1f, ((lane * (0.14f + ((0.4f * _e493) * _e499))) * _e507)));
        let _e513 = world;
        let _e517 = frame.settings.w;
        let _e523 = impactShape.a.x;
        world = mix(_e513, (anchor + (spatial * _e517)), _e523);
    }
    input.uv0_ = _e175.uv0_;
    input.custom1_ = _e175.uv1_;
    let _e532 = p.color;
    input.color = _e532;
    input.normal = normal;
    let _e535 = world;
    input.worldPosition = _e535;
    let _e540 = frame.settings.x;
    input.time = _e540;
    input.delta = 0.008333334f;
    let _e543 = input;
    let _e544 = source_vertex(_e543);
    let _e546 = p.age;
    let _e547 = magic_placeX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJORPW2YLHNFRTUOTQNRQWGZLNMVXHIX(_e544, _e546);
    let _e550 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    world = (_e547 + _e550.xyz);
    let _e557 = frame.vp;
    let _e558 = world;
    o.position = (_e557 * vec4<f32>(_e558, 1f));
    o.uv = _e175.uv0_;
    o.custom = _e175.uv1_;
    let _e568 = p.color;
    o.color = _e568;
    let _e570 = world;
    let _e573 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.anchor;
    o.world = (_e570 - _e573.xyz);
    o.normal = normal;
    let _e579 = p.id;
    let _e588 = frame.extra.x;
    let _e591 = p.id;
    let _e593 = dm_random(bitcast<u32>(_e588), _e591, 390u);
    o.bolt = vec4<f32>(_e70, (f32((_e579 % 3u)) - 1f), _e593);
    let _e595 = o;
    return _e595;
}
