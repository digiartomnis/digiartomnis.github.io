...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e203 = coatF;
    ambient = (((((kD_1 * _e192) * _e194) * skyFactor) + (_e197 * _e191)) * (vec3(1f) - _e203));
    let _e212 = probeBlendRecords[(0u + 1u)];
    let _e217 = probeBlendRecords[(0u + 2u)];
    let _e222 = probeBlendRecords[(0u + 3u)];
    let _e227 = probeBlendRecords[(0u + 4u)];
    let _e232 = probeBlendRecords[(0u + 5u)];
    let _e237 = probeBlendRecords[(0u + 6u)];
    let _e242 = probeBlendRecords[(0u + 7u)];
    let _e247 = probeBlendRecords[(0u + 8u)];
    let _e252 = probeBlendRecords[(0u + 9u)];
    let probeShPreblend = array<vec4<f32>, 9>(_e212, _e217, _e222, _e227, _e232, _e237, _e242, _e247, _e252);
    let probeLocalBlendFraction = probeBlendRecords[0u].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_12)), vec3(0.0001f)));
    let _e265 = physicalNormal;
    let _e266 = irradiance;
    let _e268 = diffuseAlbedo;
    let _e269 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e265, (_e266 * skyFactor), probeDiffuseK, _e268, metallic_12);
    if true {
        let _e271 = specularIbl;
        let _e276 = coatF;
        ambient = ((_e269 + (_e271 * _e191)) * (vec3(1f) - _e276));
    }
    let _e279 = ambient;
    ambient = (_e279 * ao);
    let _e281 = ambient;
    let _e285 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e288 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e285, ssaoBlurredTexture, ssaoBlurredSampler);
    ambient = (_e281 * _e288);
    let _e290 = ambient;
    color = _e290;
    let _e292 = color;
    color = (_e292 + _e6.emissive);
    let _e295 = physicalNormal;
    let _e297 = standardViewZ(in_1);
    let _e298 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e295, in_1.worldPos, _e297);
    let _e299 = physicalNormal;
    let _e300 = diffuseAlbedo;
    let _e301 = f0_;
    let _e302 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e299, v_2, _e300, metallic_12, a_2, _e301);
    let _e303 = color;
    let _e308 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowOrigin;
    let _e312 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowRight;
    let _e316 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowUp;
    let _e320 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cloudShadowProjection;
    let _e321 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((_e298 * _e302), in_1.worldPos, _e308.xyz, _e312.xyz, _e316.xyz, _e320);
    color = (_e303 + _e321);
    let _e323 = color;
    let _e326 = standardViewZ(in_1);
    let _e328 = physicalNormal;
    let _e329 = diffuseAlbedo;
    let _e330 = f0_;
    let _e333 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e326, in_1.worldPos, _e328, v_2, _e329, metallic_12, a_2, _e330, false, true);
    color = (_e323 + _e333);
    let _e335 = color;
    return vec4<f32>(_e335, alpha);
}
